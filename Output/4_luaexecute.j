...
real subAttrFixedBonus; // 次属性固定加成
 ...
 public static trigger trStrChange = null; /* ATTR变化触发器 */
 ...
 StrRateBonus = getBaseStr() * getCurrentStrRate(); 
 ...
 public static trigger trAgiChange = null; /* ATTR变化触发器 */
 
 /* 获取基础ATTR(白字) */
 public method getBaseAgi() -> real { 
 if (mainAttrType == MAIN_ATTR_AGI) { 
 ...
 /* 获取额外ATTR(绿字) */
 public method getExtraAgi() -> real { 
 if (mainAttrType == MAIN_ATTR_AGI) { 
 ...
 AgiRateBonus = getBaseAgi() * getCurrentAgiRate(); 
 SetHeroAgi(u, R2I(RMaxBJ(getCurrentAgi(), 0.0)), true); 
 if (trAgiChange != null) { 
 ethis = this; 
 TriggerEvaluate(trAgiChange); 
 } 
 } 
 ...
 /* 回调ATTR变化 */
 public static method onAgiChange(code func) { 
 if (trAgiChange == null) { 
 trAgiChange = CreateTrigger(); 
 } 
 TriggerAddCondition(trAgiChange, Condition(func)); 
 } 
 ...
 public static trigger trIntChange = null; /* ATTR变化触发器 */
 
 /* 获取基础ATTR(白字) */
 public method getBaseInt() -> real { 
 if (mainAttrType == MAIN_ATTR_INT) { 
 ...
 /* 获取额外ATTR(绿字) */
 public method getExtraInt() -> real { 
 if (mainAttrType == MAIN_ATTR_INT) { 
 ...
 IntRateBonus = getBaseInt() * getCurrentIntRate(); 
 SetHeroInt(u, R2I(RMaxBJ(getCurrentInt(), 0.0)), true); 
 if (trIntChange != null) { 
 ethis = this; 
 TriggerEvaluate(trIntChange); 
 } 
 } 
 ...
 /* 回调ATTR变化 */
 public static method onIntChange(code func) { 
 if (trIntChange == null) { 
 trIntChange = CreateTrigger(); 
 } 
 TriggerAddCondition(trIntChange, Condition(func)); 
 } 
        ...
        static method getMainAttr (unit u) -> integer {
            ...
            if (IsHeroUnitId(GetUnitTypeId(u))) {
                if (ha.isExist()) {
                    ...
                    primary = (EXExecuteScript("(require'jass.slk').unit[" + I2S( GetUnitTypeId(u)) + "]." + "Primary"));
                    if (primary == "STR") {
                        primary = null;
...
library Tooltip requires Icon {
    public struct tooltip {
        ...
        icon ic[50];
        uiText text[50];
        boolean alignWidth[50]; //是否对齐宽度
integer iconCount;
        integer textCount;
        integer relative; //整个框架的锚点控制(下面的UI)
integer relativeTop; //整个框架的锚点控制(上面的UI)
        method isExist () -> boolean {return (this != null && si__tooltip_V[this] == -1);}
        //懒加载函数
...
text[2] = uiText.create(border.ui)
            ...
            text[1] = uiText.create(border.ui)
                .setText(titleText)
                .setPoint(7,text[2].ui,1,0,0.005);
            textCount = 2;
            relative = text[2].ui; //锚点控制
relativeTop = text[1].ui; //锚点控制
alignWidth[2] = true; //对齐宽度
border.setPoint(1, text[1].ui, 1, 0, 0.01);
            border.setPoint(7, text[2].ui, 7, 0, -0.01);
            border.setPointFix(3, text[2].ui, 3, -0.01, 0);
            border.setPointFix(5, text[2].ui, 5, 0.01, 0);
            this.setWidth(0.2);
            //以底描述为基准
...
text[1] = uiText.create(border.ui)
            ...
            //初始化border位置
            border.setPoint(1, text[1].ui, 1, 0, 0.01);
            border.setPoint(7, text[1].ui, 7, 0, -0.01);
            border.setPointFix(3, text[1].ui, 3, -0.01, 0);
            border.setPointFix(5, text[1].ui, 5, 0.01, 0);
            return this;
        }
        //添加文本到tooltip顶部
        method addText(string content) -> uiText {
            ...
            //设置新文本的位置
            text[newPosition].setPoint(7, text[newPosition-1].ui, 1, 0, 0.005);
            //更新border边界(头部)
            border.setPoint(1, text[textCount].ui, 1, 0, 0.01);
...
static if (LIBRARY_UILifeCycle) {uiLifeCycle.onCreateCB(this,thistype.typeid,ui);} 
 static if (LIBRARY_UIHashTable) {uiHashTable(ui).ui.bind(thistype.typeid,this); }
        ...
        static method createRC (integer parent) -> thistype {
            thistype this = allocate();
            id = uiId.get();
            ui = DzCreateFrameByTagName("GLUEBUTTON","Btn" + I2S(id),parent,"TEMPLATE_TEXT_BUTTON",0); //配合异度下的菜单使用,要导入:ui\image\textbutton_highlight.blp
static if (LIBRARY_UILifeCycle) {uiLifeCycle.onCreateCB(this,thistype.typeid,ui);} 
 static if (LIBRARY_UIHashTable) {uiHashTable(ui).ui.bind(thistype.typeid,this); }
            return this;
        }
 ...
 static if (LIBRARY_UIHashTable) {uiHashTable(ui).ui.bind(thistype.typeid,this); }
        ...
        static method createMenu (integer parent) -> thistype {
            thistype this = allocate();
            id = uiId.get();
            ui = DzCreateFrameByTagName("TEXTBUTTON","Btn" + I2S(id),parent,"TestButton1",0);
            static if (LIBRARY_UILifeCycle) {uiLifeCycle.onCreateCB(this,thistype.typeid,ui);} 
 static if (LIBRARY_UIHashTable) {uiHashTable(ui).ui.bind(thistype.typeid,this); }
            return this;
        }
...
string units = ""; // 单位字符串
    ...
    public function FormatSimple (real r) -> string {
        real wan = r / 10000.0;
        real yi = r / 100000000.0;
        if (r < 10000.0) return I2S(R2I(r));
    ...
    public function FormatWithCommas (integer num) -> string {
        ...
        for (i = StringLength(result);i > 0;i -= 3) {
            temp = SubString(result, (IMaxBJ(1,i-2))-1, i) + temp;
            if (i > 3) {temp = "," + temp;}
        }
        if (num <0 ) {
            temp = "-" + temp;
    ...
    public function ParseReal (string s) -> real {
        ...
        lastWord = SubString(s, (len)-1, len);
        if (lastWord == "w" || lastWord == "W") {return S2R(SubString(s, (1)-1, len-1)) * 10000.;} //万
else if (lastWord == "k" || lastWord == "K") {return S2R(SubString(s, (1)-1, len-1)) * 1000.;} //千
else if (lastWord == "e" || lastWord == "E") {return S2R(SubString(s, (1)-1, len-1)) * 100000000.;} //亿
        return S2R(s);
    }
    //测试用:将输入的值转成值返回
...
optional module allUnitAttr; //其他地图的自定义属性
 ...
 this.SpellDmgRateDown = 0.0; 
			...
			this.FinalDmgRateUp = 0.0; 
 this.FinalDmgRateDown = 0.0; 

			// 初始化魔抗
			this.ResistRateUp = 0.0; 
 this.ResistRateDown = 0.0; 

			// 初始化移速
			this.baseMoveSpeed = 0.0; 
 this.MoveSpeedRateUp = 0.0; 
 this.MoveSpeedRateDown = 0.0; 
 this.MoveSpeedRateBonus = 0.0; 
 this.MoveSpeedFixedBonus = 0.0; 
...
this.initAllUnitAttr();
 ...
 return (1.0 + SpellDmgRateUp) * (1.0 - SpellDmgRateDown) - 1.0; 
		...
		public real FinalDmgRateUp; /* ATTR增幅比例 */
 public real FinalDmgRateDown; /* ATTR减幅比例 */
 
 /* 增加ATTR增幅比例 */
 public method addFinalDmgRateUp(real value) { 
 if (value != 0) { 
 FinalDmgRateUp += value; 
 } 
 } 
 
 /* 增加ATTR减幅比例 */
 public method addFinalDmgRateDown(real value) { 
 if (value != 0) { 
 FinalDmgRateDown = RealAdd(FinalDmgRateDown, value); 
 } 
 } 
 
 /* 获取当前的ATTR最终倍率 */
 public method getCurrentFinalDmg() -> real { 
 return (1.0 + FinalDmgRateUp) * (1.0 - FinalDmgRateDown) - 1.0; 
 } 

		// 移速(类攻击)
		
 public real baseMoveSpeed; /* 基础ATTR值 */
 public real MoveSpeedRateUp; /* ATTR增幅比例 */
 public real MoveSpeedRateDown; /* ATTR减幅比例 */
 public real MoveSpeedRateBonus; /* 受增减幅影响的bonus值 */
 public real MoveSpeedFixedBonus;/* 固定加成值(不受增减幅影响) */
 public static trigger trMoveSpeedChange = null; /* ATTR变化触发器 */
 
 /* 设置基础ATTR */
 public method setBaseMoveSpeed(real value) { 
 if (baseMoveSpeed != value) { 
 baseMoveSpeed = value; 
 syncMoveSpeedRate(); 
 } 
 } 
 ...
 /* 增加基础ATTR */
 public method addBaseMoveSpeed(real value) { 
 if (value != 0) { 
 baseMoveSpeed += value; 
 syncMoveSpeedRate(); 
 } 
 ...
 /* 增加固定bonus */
 public method addMoveSpeedFixedBonus(real value) { 
 if (value != 0) { 
 MoveSpeedFixedBonus += value; 
 syncMoveSpeedRate(); 
 } 
 } 
 
 /* 增加ATTR增幅 */
 public method addMoveSpeedRateUp(real value) { 
 if (value != 0) { 
 MoveSpeedRateUp += value; 
 syncMoveSpeedRate(); 
 } 
 } 
 
 /* 增加ATTR减幅 */
 public method addMoveSpeedRateDown(real value) { 
 if (value != 0) { 
 MoveSpeedRateDown = RealAdd(MoveSpeedRateDown, value); 
 syncMoveSpeedRate(); 
 } 
 } 
 
 /* 获取当前总ATTR */
 public method getCurrentMoveSpeed() -> real { 
 return baseMoveSpeed + MoveSpeedRateBonus + MoveSpeedFixedBonus; 
 } 
 
 /* 获取当前ATTR倍率 */
 public method getCurrentMoveSpeedRate() -> real { 
 return (1.0 + MoveSpeedRateUp) * (1.0 - MoveSpeedRateDown) - 1.0; 
 } 
 
 /* 回调ATTR变化 */
 public static method onMoveSpeedChange(code func) { 
 if (trMoveSpeedChange == null) { 
 trMoveSpeedChange = CreateTrigger(); 
 } 
 TriggerAddCondition(trMoveSpeedChange, Condition(func)); 
 } 

		// 使用宏定义生成概率性属性(魔抗)
		public real ResistRateUp; /* ATTR增幅比例 */
 public real ResistRateDown; /* ATTR减幅比例 */
 
 /* 增加ATTR增幅比例 */
 public method addResistRateUp(real value) { 
 if (value != 0) { 
 ResistRateUp = RealAdd(ResistRateUp, value); 
 } 
 } 
 
 /* 增加ATTR减幅比例 */
 public method addResistRateDown(real value) { 
 if (value != 0) { 
 ResistRateDown = RealAdd(ResistRateDown, value); 
 } 
 } 
 
 /* 获取当前的ATTR最终倍率,这里不用减1.0,和上面的不一样 */
 public method getCurrentResist() -> real { 
 return ResistRateUp * (1.0 - ResistRateDown); 
 } 
...
MNum = MNum + 1;
			...
			DzFrameSetPoint(ui,anchor1,align,anchor2,Cos((angle)*0.0174538)*off,Sin((angle)*0.0174538)*off);
			UIA.reg(); //Add了后就调用了这个自动开始
}
...
DzFrameClearAllPoints(ui);
								DzFrameSetPoint(ui,anchor1,align,anchor2,Cos((angle)*0.0174538)*(off +dist),Sin((angle)*0.0174538)*(off +dist));
								MList[i] = MList[MNum];
								MList[i].mID = i;
								MNum = MNum - 1;
								i = i - 1;
								...
								mNow = mNow + 1;
								DzFrameClearAllPoints(ui);
								DzFrameSetPoint(ui,anchor1,align,anchor2,Cos((angle)*0.0174538)*(off +dist * mNow / mTime),Sin((angle)*0.0174538)*(off +dist * mNow / mTime));
							}
						} //还在延迟中不进行操作
}
				}
...
iconAttack = icon.fromExistingUI(uiImage.bindSimple("单位面板图标", 0), parent)
                ...
                .setTexture("ui\\console\\unitpanel\\origin_attack.blp");
            ...
            textAttackExtra = uiText.createSimple(parent)
                .setPoint(3, textAttackValue.ui, 5, 0.002, 0.0);
            //防御小框架相关
...
iconArmor = icon.fromExistingUI(uiImage.bindSimple("单位面板图标", 1), parent)
                ...
                .setTexture("ui\\console\\unitpanel\\origin_armor.blp");
            ...
            textArmorExtra = uiText.createSimple(parent)
                .setPoint(3, textArmorValue.ui, 5, 0.002, 0.0);
            //英雄属性三围
...
iconHero = icon.fromExistingUI(uiImage.bindSimple("英雄三围图标", 0), parent)
                ...
                .setTexture("ui\\console\\unitpanel\\origin_str.blp");
            ...
            textStrExtra = uiText.createSimple(parent)
                .setPoint(3, textStrValue.ui, 5, 0.002, 0.0);
            //敏捷
            ...
            textAgiExtra = uiText.createSimple(parent)
                .setPoint(3, textAgiValue.ui, 5, 0.002, 0.0);
            //智力
            ...
            textIntExtra = uiText.createSimple(parent)
                .setPoint(3, textIntValue.ui, 5, 0.002, 0.0);
            //建筑小框架相关
...
textExp = uiText.createSimple(DzSimpleFrameFindByName("SimpleInfoPanelIconArmor", 2))
        ...
        //隐藏/显示额外数值显示
    //隐藏/显示额外数值显示
    static method showAttackExtra (boolean flag) { if (flag) textAttackExtra.setPoint(3, textAttackValue.ui, 5, 0.002, 0.0); else textAttackExtra.setPoint(3, textAttackValue.ui, 5, -2.0, 0.0); }
    static method showArmorExtra (boolean flag) { if (flag) textArmorExtra.setPoint(3, textArmorValue.ui, 5, 0.002, 0.0); else textArmorExtra.setPoint(3, textArmorValue.ui, 5, -2.0, 0.0); }
    static method showStrExtra (boolean flag) { if (flag) textStrExtra.setPoint(3, textStrValue.ui, 5, 0.002, 0.0); else textStrExtra.setPoint(3, textStrValue.ui, 5, -2.0, 0.0); }
    static method showAgiExtra (boolean flag) { if (flag) textAgiExtra.setPoint(3, textAgiValue.ui, 5, 0.002, 0.0); else textAgiExtra.setPoint(3, textAgiValue.ui, 5, -2.0, 0.0); }
    static method showIntExtra (boolean flag) { if (flag) textIntExtra.setPoint(3, textIntValue.ui, 5, 0.002, 0.0); else textIntExtra.setPoint(3, textIntValue.ui, 5, -2.0, 0.0); }
    //把所有原生UI移走
...
library ToolTipStack {
    ...
    public struct tooltipStack []{
        ...
        static method pushOrigin () {
            DzFrameSetAbsolutePoint(DzFrameGetTooltip(),8,0,0); //移走
push(function(player p) {
                DzFrameSetAbsolutePoint(DzFrameGetTooltip(),8,.8,.1625); //移回
});
        }
        // 获取当前栈大小
...
library UIBorder requires UIId,UITocInit,UIBaseModule,UIImageModule {
    public struct uiBorder {
        ...
        integer ui; 
 integer id; 
 method isExist () -> boolean {return (this != null && si__uiBorder_V[this] == -1);} 
 optional module uiLifeCycle; 
 module uiBaseModule;
        module uiImageModule; // UI图片的共用方法
        // 创建边框种类1
        ...
        static method create (integer parent) -> thistype {
            thistype this = allocate();
            id = uiId.get();
            ui = DzCreateFrameByTagName("BACKDROP","Img" + I2S(id),parent,"Border1",0);
            static if (LIBRARY_UILifeCycle) {uiLifeCycle.onCreateCB(this,thistype.typeid,ui);} 
 static if (LIBRARY_UIHashTable) {uiHashTable(ui).ui.bind(thistype.typeid,this); }
            return this;
        }
        ...
        static method createType2 (integer parent) -> thistype {
            thistype this = allocate();
            id = uiId.get();
            ui = DzCreateFrameByTagName("BACKDROP","Img" + I2S(id),parent,"Border2",0);
            static if (LIBRARY_UILifeCycle) {uiLifeCycle.onCreateCB(this,thistype.typeid,ui);} 
 static if (LIBRARY_UIHashTable) {uiHashTable(ui).ui.bind(thistype.typeid,this); }
            return this;
        }
        ...
        static method createType3 (integer parent) -> thistype {
            thistype this = allocate();
            id = uiId.get();
            ui = DzCreateFrameByTagName("BACKDROP","Img" + I2S(id),parent,"Border3",0);
            static if (LIBRARY_UILifeCycle) {uiLifeCycle.onCreateCB(this,thistype.typeid,ui);} 
 static if (LIBRARY_UIHashTable) {uiHashTable(ui).ui.bind(thistype.typeid,this); }
            return this;
        }
        ...
        static method createType4 (integer parent) -> thistype {
            thistype this = allocate();
            id = uiId.get();
            ui = DzCreateFrameByTagName("BACKDROP","Img" + I2S(id),parent,"Border4",0);
            static if (LIBRARY_UILifeCycle) {uiLifeCycle.onCreateCB(this,thistype.typeid,ui);} 
 static if (LIBRARY_UIHashTable) {uiHashTable(ui).ui.bind(thistype.typeid,this); }
            return this;
        }
 ...
 static if (LIBRARY_UIHashTable) {uiHashTable(ui).ui.bind(thistype.typeid,this); }
        ...
        method alignParent(integer ui,real padding) -> thistype {
            if (!this.isExist()) {return this;}
            this.setPoint(0, ui, 0, -1 * padding, padding);
            this.setPoint(8, ui, 8, padding, -1 * padding);
            return this;
        }
        method onDestroy () {
            if (!this.isExist()) {return;}
...
unitPanel.showAttackExtra(false);
                unitPanel.showArmorExtra(false);
                unitPanel.textAttackValue.setText(FormatNumber(GetUnitState(u,ConvertUnitState(0x12))));
                unitPanel.textArmorValue.setText(FormatNumber(GetUnitState(u,ConvertUnitState(0x20))));
            }
            if (IsHeroUnitId(GetUnitTypeId(u))) {
                mainAttr = heroAttr.getMainAttr(u);
                if (mainAttr ==MAIN_ATTR_STR) {
                    unitPanel.iconHero.setTexture("ui\\console\\unitpanel\\origin_str.blp");
                } else if (mainAttr ==MAIN_ATTR_AGI) {
                    unitPanel.iconHero.setTexture("ui\\console\\unitpanel\\origin_agi.blp");
                } else if (mainAttr ==MAIN_ATTR_INT) {
                    unitPanel.iconHero.setTexture("ui\\console\\unitpanel\\origin_int.blp");
                }
                if (ha.isExist()){
                    updateStr(ha);
                    updateAgi(ha);
...
attack = GetUnitState(DzGetSelectedLeaderUnit(),ConvertUnitState(0x12));
                range = GetUnitState(DzGetSelectedLeaderUnit(),ConvertUnitState(0x16));
                attackSpeed = GetUnitState(DzGetSelectedLeaderUnit(),ConvertUnitState(0x51));
                attackInterval = GetUnitState(DzGetSelectedLeaderUnit(),ConvertUnitState(0x25));
            }
...
armor = GetUnitState(DzGetSelectedLeaderUnit(),ConvertUnitState(0x20));
                resist = 0.0;
                moveSpeed = GetUnitMoveSpeed(DzGetSelectedLeaderUnit());
            }
            armorEffect = (armor * 0.01) / (1 + 0.01 * armor);
			tooltipStack.pushOrigin();
			if (tt.isExist()) {
				tt.destroy();
			}
...
uiHTFrame ui ; //方便链式调用  uiHashTable(frame).ui.bind(8174);
    ...
    struct uiHTEvent [] {
        ...
        method bind2 (integer value) {
            SaveInteger(HASH_UI,frame,1824,value);
        }
        method get2 () -> integer {
            return LoadInteger(HASH_UI,frame,1824);
        }
    }
}
//! endzinc
	...
	function onInit () {
		...
		UnitTestRegisterChatEvent(function () {
			...
			if (SubString(str, (1)-1, 1) == "-") {
				TTestActUTUnitAttrUpdate1(SubString(str, (2)-1, StringLength(str)));
				return;
			}
