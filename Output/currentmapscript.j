...
//library GroupUtils:
    ...
    function CopyGroup takes group g1 returns group  //复制g1进新组并返回
        set GroupUtils__tempG=CreateGroup()
...
//library CameraControl:
    ...
    //! pragma implicitthis
        static method openWheel takes nothing returns nothing  // 锁定镜头高度
            call DoNothing()
        endmethod
        static method lockHeight takes nothing returns nothing  // 解锁镜头高度
            set CameraControl__HeightLocked=true
        endmethod
        static method unlockHeight takes nothing returns nothing  // 查询是否锁定
            set CameraControl__HeightLocked=false
        endmethod
        static method isHeightLocked takes nothing returns boolean  // 初始化镜头（仅对指定玩家生效）
            return CameraControl__HeightLocked
        endmethod
        static method initCamera takes player p returns nothing
        ...
        endmethod  // 增加镜头高度400（仅对指定玩家生效）
        static method increaseHeight takes player p returns nothing
            if (GetLocalPlayer()!=p)then
                return
            endif
        ...
        endmethod  // 减少镜头高度400（仅对指定玩家生效）
        static method decreaseHeight takes player p returns nothing
            if (GetLocalPlayer()!=p)then
                return
            endif
    ...
    endstruct  // 滚轮控制镜头
        function CameraControl__anon__0 takes nothing returns nothing  // 初始化就调用 //注册滚轮事件 //滚轮变化量
            local integer delta=DzGetWheelDelta()  // 鼠标不在游戏内或焦点在UI控件上则不处理
...
//library DamageUtils:
    ...
    //! pragma implicitthis
        unit source
        string eft
        real damage  // Lifesteal aggregation control (skill lifesteal only when enabled on AoE)
        boolean enableLifestealAggregation
        real lifestealDealtTotal  // 正确使用 onDestroy，而不是 destroy
    ...
    //! pragma implicitthis
        private static DmgP  array stack
        private static integer top=-1
        static method push takes DmgP params returns nothing
        ...
        endmethod
        static method pop takes nothing returns DmgP
            local DmgP params=thistype.stack[thistype.top]
            ...
            return params
        endmethod
        static method current takes nothing returns DmgP
            return thistype.stack[thistype.top]
        endmethod  // 是否存在可用的上下文，且开启了技能吸血聚合，并且来源匹配
        static method hasAggregation takes unit src returns boolean
            local DmgP p
            if (thistype.top<0)then
                return false
            endif
            set p=thistype.stack[thistype.top]
            return p.enableLifestealAggregation and p.source==src
        endmethod  // 聚合一次最终造成的伤害（由 DamageSystem 在事件最终阶段调用）
        static method aggregate takes real amount returns nothing
            local DmgP p
            if (thistype.top<0)then
                return
            endif
            set p=thistype.stack[thistype.top]
            if (p.enableLifestealAggregation and amount>0)then
                set p.lifestealDealtTotal=p.lifestealDealtTotal+amount  // 写回（结构体为值语义）
                set thistype.stack[thistype.top]=p
            endif
        endmethod
    endstruct  // 范围普通伤害
        function DamageUtils__anon__0 takes nothing returns boolean
            local DmgP current=DmgS.current()
    ...
    function DamageAreaPhysical takes unit u,real x,real y,real radius,real damage,string efx returns nothing
        ...
        local DmgP params
        set g=CreateGroup()
        set params=DmgP.create()
        set params.source=u
        set params.eft=efx
        set params.damage=damage
        set params.enableLifestealAggregation=(DamageUtils__TrLifeSteal!=null)
        set params.lifestealDealtTotal=0.0
        call DmgS.push(params)
        call GroupEnumUnitsInRangeEx(g,x,y,radius,Filter(function DamageUtils__anon__0))
        set params=DmgS.pop()
        set dealt=params.lifestealDealtTotal
        call params.destroy()
        call DestroyGroup(g)
    ...
    function DamageAreaPure takes unit u,real x,real y,real radius,real damage,string efx returns nothing
        ...
        function DamageUtils__anon__3 takes nothing returns boolean
            local DmgP current=DmgS.current()
            if (IsEnemy(GetFilterUnit(),GetOwningPlayer(current.source)))then
                call UnitDamageTarget(current.source,GetFilterUnit(),GetUnitState(GetFilterUnit(),UNIT_STATE_MAX_LIFE)*1.2,false,true,ATTACK_TYPE_CHAOS,DAMAGE_TYPE_SLOW_POISON,WEAPON_TYPE_WHOKNOWS)
                if (current.eft!=null)then
                    call DestroyEffect(AddSpecialEffect(current.eft,GetUnitX(GetFilterUnit()),GetUnitY(GetFilterUnit())))
                endif
                return true
    ...
    function GetDamageLifeStealUnit takes nothing returns unit  //吸血的数值
...
//library UTDamageUtils:
        ...
        function UTDamageUtils__anon__0 takes nothing returns nothing  // 清理所有已存在的测试单位
    ...
    function UTDamageUtils__TTestUTDamageUtils8 takes player p returns nothing
        ...
        function UTDamageUtils__anon__1 takes nothing returns boolean  // 注册吸血回调
            local unit u
            local real damage
            local real heal
    ...
    function UTDamageUtils__TTestActUTDamageUtils1 takes string str returns nothing
        ...
        function UTDamageUtils__anon__3 takes nothing returns nothing  // 创建伤害事件触发器
            local unit source=GetEventDamageSource()
            local unit target=GetTriggerUnit()
            local real damage=GetEventDamage()  // 显示伤害信息
            if (UTDamageUtils__isShowDamage)then
                call Trace("|cffff0000伤害事件|r - 来源: "+GetUnitName(source)+" 目标: "+GetUnitName(target)+"("+I2S(GetHandleId(target))+") 伤害: "+R2S(damage)+" 当前栈层: "+I2S(DmgS.current()))
            endif  // 反伤测试
            if (UTDamageUtils__isReflectDamage and UTDamageUtils__reflectCount<5)then  // 限制反伤次数 // 增加反伤计数
                ...
                call DamageAreaPhysical(target,GetUnitX(target),GetUnitY(target),100,damage*0.5,I2S(DmgS.current()))
                if (UTDamageUtils__reflectCount>=5)then
                    call Trace("|cffff0000达到最大反伤次数(5次),现在栈层: "+I2S(DmgS.current()))
                endif
            endif  // 单体聚合（LS 栈）
            if (DamageUtils.LS_isActive(source))then
                call DamageUtils.LS_add(GetEventDamage())
                call Trace("|cff00ff00吸血聚合|r - 来源: "+GetUnitName(source)+" 伤害: "+R2S(damage))
            endif  // AoE 聚合（DmgS 栈）
            if (DamageUtils.DmgS.hasAggregation(source))then
                call DamageUtils.DmgS.aggregate(GetEventDamage())
