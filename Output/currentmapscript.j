...
//globals from EasingUtils:
constant boolean LIBRARY_EasingUtils=true
//endglobals from EasingUtils
...
//globals from ToastPop:
constant boolean LIBRARY_ToastPop=true
//endglobals from ToastPop
//globals from UTToastPop:
constant boolean LIBRARY_UTToastPop=true
//endglobals from UTToastPop
    // Generated
...
constant integer si__uiImage=7
...
constant integer si__toastPop=9
integer si__toastPop_F=0
integer si__toastPop_I=0
integer array si__toastPop_V
integer array s__toastPop_List
integer s__toastPop_size=0
integer s__toastPop_UIA=0
integer array s__toastPop_bg
integer array s__toastPop_icon
integer array s__toastPop_title
integer array s__toastPop_content
integer array s__toastPop_cd
integer array s__toastPop_totalTime
real array s__toastPop_length
integer array s__toastPop_id
trigger st__uiImage_onDestroy
trigger st__uiText_onDestroy
trigger st__toastPop_onDestroy
trigger st__toastPop_initAnimation
real f__arg_real1
real f__arg_real2
integer f__arg_this
...
//Generated method caller for toastPop.onDestroy
function sc__toastPop_onDestroy takes integer this returns nothing
    set f__arg_this=this
    call TriggerEvaluate(st__toastPop_onDestroy)
endfunction

//Generated method caller for toastPop.initAnimation
function sc__toastPop_initAnimation takes integer this,real length,real time returns integer
    set f__arg_this=this
    set f__arg_real1=length
    set f__arg_real2=time
    call TriggerEvaluate(st__toastPop_initAnimation)
 return f__result_integer
endfunction

//Generated allocator of toastPop
function s__toastPop__allocate takes nothing returns integer
 local integer this=si__toastPop_F
    if (this!=0) then
        set si__toastPop_F=si__toastPop_V[this]
    else
        set si__toastPop_I=si__toastPop_I+1
        set this=si__toastPop_I
    endif
    if (this>8190) then
        call DisplayTimedTextToPlayer(GetLocalPlayer(),0,0,1000.,"Unable to allocate id for an object of type: toastPop")
        return 0
    endif

   set s__toastPop_bg[this]=0 // 背景框 
   set s__toastPop_icon[this]=0
   set s__toastPop_title[this]=0
   set s__toastPop_content[this]=0
    set si__toastPop_V[this]=-1
 return this
endfunction

//Generated destructor of toastPop
function sc__toastPop_deallocate takes integer this returns nothing
    if this==null then
            call DisplayTimedTextToPlayer(GetLocalPlayer(),0,0,1000.,"Attempt to destroy a null struct of type: toastPop")
        return
    elseif (si__toastPop_V[this]!=-1) then
            call DisplayTimedTextToPlayer(GetLocalPlayer(),0,0,1000.,"Double free of type: toastPop")
        return
    endif
    set f__arg_this=this
    call TriggerEvaluate(st__toastPop_onDestroy)
    set si__toastPop_V[this]=si__toastPop_F
    set si__toastPop_F=this
endfunction
...
//library BzAPI ends
//library EasingUtils:
    function EaseOutBack takes real x returns real
        return 1 + 2.70158 * Pow(x - 1, 3) + 1.70158 * Pow(x - 1, 2)
    endfunction
    function EaseOutBack2 takes real x,real rate returns real
        local real nx=x * rate
        if ( nx >= 1.0 ) then
            return 1.0
        else
            return EaseOutBack(nx)
        endif
    endfunction
    function EaseInBack takes real x returns real
        return 2.70158 * x * x * x - 1.70158 * x * x
    endfunction
    function EaseInOutBack takes real x returns real
        if ( x < 0.5 ) then
            return ( Pow(2 * x, 2) * ( ( 2.5949095 + 1 ) * 2 * x - 2.5949095 ) ) / 2
        else
            return ( Pow(2 * x - 2, 2) * ( ( 2.5949095 + 1 ) * ( x * 2 - 2 ) + 2.5949095 ) + 2 ) / 2
        endif
    endfunction
    function EaseInOutCirc takes real x returns real
        if ( x < 0.5 ) then
            return ( 1 - SquareRoot(1 - Pow(2 * x, 2)) ) / 2
        else
            return ( SquareRoot(1 - Pow(- 2 * x + 2, 2)) + 1 ) / 2
        endif
    endfunction
    function EaseOutExpo takes real x returns real
        if ( x >= 1. ) then
            return 1.
        else
            return 1 - Pow(2, - 10 * x)
    ...
    function EaseInExpo takes real x returns real
        if ( x <= 0. ) then
            return 0.
        else
    ...
    function EaseOutCubic takes real x returns real
        return 1 - Pow(1 - x, 3)
    endfunction
    function EaseOutBounce takes real x returns real
        if ( x < 1 / 2.75 ) then
            return 7.5625 * x * x
    ...
    function EaseParabolic takes real x returns real
        return - 4 * x * x + 4 * x
    endfunction
    function EasePartialParabolic takes real x returns real
        if ( x < 0.5 and x > 0. ) then
            return - 16 * x * x + 8 * x
        endif
        return 0.
    endfunction
    function EaseTriangle takes real x returns real
        if ( x <= 0.5 ) then
            return 2 * x
        else
            return 2 - 2 * x
    ...
    function EaseSineWave takes real x returns real
        return Sin(x * bj_PI)
    endfunction
    function EaseInQuart takes real x returns real
        return x * x * x * x
    endfunction
    function EaseOutQuart takes real x returns real
        return 1 - Pow(1 - x, 4)
    endfunction
    function EaseInOutQuart takes real x returns real
        if ( x < 0.5 ) then
            return 8 * x * x * x * x
        endif
        return 1 - Pow(- 2 * x + 2, 4) / 2
    endfunction
    function EaseInCubic takes real x returns real
        return x * x * x
    endfunction
    function EaseInOutCubic takes real x returns real
        if ( x < 0.5 ) then
            return 4 * x * x * x
        endif
        return 1 - Pow(- 2 * x + 2, 3) / 2
    endfunction
    function EaseInQuad takes real x returns real
        return x * x
    endfunction
    function EaseOutQuad takes real x returns real
        return 1 - ( 1 - x ) * ( 1 - x )
    endfunction
    function EaseInOutQuad takes real x returns real
        if ( x < 0.5 ) then
            return 2 * x * x
        endif
        return 1 - Pow(- 2 * x + 2, 2) / 2
    endfunction
    function EaseInSine takes real x returns real
        return 1 - Cos(( x * bj_PI ) / 2)
    endfunction
    function EaseOutSine takes real x returns real
        return Sin(( x * bj_PI ) / 2)
    endfunction
    function EaseInOutSine takes real x returns real
        return - ( Cos(bj_PI * x) - 1 ) / 2
    endfunction

//library EasingUtils ends
//library LBKKAPI:
...
//Implemented from module uiBaseModule:
        ...
        function s__uiImage_setSizeFixVertical takes integer this,real width,real height returns integer
            if ( not ( s__uiImage_isExist(this) ) ) then
                return this
            endif
            call DzFrameSetSize(s__uiImage_ui[this], width, height / GetResizeRate())
            return this
        endfunction  // 显示控件
...
//#                 call uiHashTable(ui).ui.bind(uiImage.typeid,this)
//#             endif
            return this
...
//Implemented from module uiBaseModule:
        ...
        function s__uiText_setSizeFixVertical takes integer this,real width,real height returns integer
            if ( not ( s__uiText_isExist(this) ) ) then
                return this
            endif
            call DzFrameSetSize(s__uiText_ui[this], width, height / GetResizeRate())
            return this
        endfunction  // 显示控件
...
//#             endif
//#             static if LIBRARY_UIHashTable then
//#                 call uiHashTable(ui).ui.bind(uiText.typeid,this)
//#             endif
            return this
...
//library UIText ends
//library ToastPop:
        function s__toastPop_onDestroy takes integer this returns nothing
            if ( s__toastPop_bg[this] != 0 ) then
                call s__uiImage_deallocate(s__toastPop_bg[this])
            endif
            if ( s__toastPop_icon[this] != 0 ) then
                call s__uiImage_deallocate(s__toastPop_icon[this])
            endif
            if ( s__toastPop_title[this] != 0 ) then
                call s__uiText_deallocate(s__toastPop_title[this])
            endif
            if ( s__toastPop_content[this] != 0 ) then
                call s__uiText_deallocate(s__toastPop_content[this])
            endif
            set s__toastPop_bg[this]=0
            set s__toastPop_icon[this]=0
            set s__toastPop_title[this]=0
            set s__toastPop_content[this]=0
            set s__toastPop_cd[this]=0
            set s__toastPop_totalTime[this]=0
            set s__toastPop_length[this]=0 // 从列表移除
            if ( s__toastPop_id[this] != 0 ) then
                set s__toastPop_List[s__toastPop_id[this]]=s__toastPop_List[s__toastPop_size]
                set s__toastPop_id[s__toastPop_List[s__toastPop_id[this]]]=s__toastPop_id[this]
                set s__toastPop_size=s__toastPop_size - 1
                set s__toastPop_id[this]=0
            endif // 停止动画
            if ( s__toastPop_size <= 0 ) then
                call s__uianim_unreg(s__toastPop_UIA)
                call BJDebugMsg("停止ToastPop")
            endif
        endfunction  // 初始化基础UI框架(内部方法)

//Generated destructor of toastPop
function s__toastPop_deallocate takes integer this returns nothing
    if this==null then
        call DisplayTimedTextToPlayer(GetLocalPlayer(),0,0,1000.,"Attempt to destroy a null struct of type: toastPop")
        return
    elseif (si__toastPop_V[this]!=-1) then
    ...
    call s__toastPop_onDestroy(this)
    set si__toastPop_V[this]=si__toastPop_F
    set si__toastPop_F=this
endfunction
        function s__toastPop_initBase takes integer this,player p returns integer
            local real resizeX
            if ( GetLocalPlayer() != p ) then
                return 0
            endif
            set this=s__toastPop__allocate()
            set resizeX=GetResizeRate() // 创建背景框
            set s__toastPop_bg[this]=s__uiImage_setAbsPoint(s__uiImage_setSize(s__uiImage_setTexture(s__uiImage_create(DzGetGameUI()),"ui\\image\\BG_PopUI.blp"),0.3 , 0.3 / resizeX * 0.29032),ANCHOR_BOTTOM , .4 , .1375)
            return this
        endfunction  // 创建带图标的弹窗
        function s__toastPop_createWithIcon takes player p,string iconPath,string contentText returns integer
            local integer this=0
            local real resizeX=GetResizeRate()
            set this=s__toastPop_initBase(this,p)
            if ( this == 0 ) then // 创建图标
                return 0
            endif
            set s__toastPop_icon[this]=s__uiImage_setPoint(s__uiImage_setSize(s__uiImage_setTexture(s__uiImage_create(s__uiImage_ui[s__toastPop_bg[this]]),iconPath),0.03 , 0.03 / resizeX),ANCHOR_TOP , s__uiImage_ui[s__toastPop_bg[this]] , ANCHOR_TOP , 0 , - 0.012 / resizeX)
            set s__toastPop_content[this]=s__uiText_setText(s__uiText_setPoint(s__uiText_setPoint(s__uiText_setPoint(s__uiText_setPoint(s__uiText_create(s__uiImage_ui[s__toastPop_bg[this]]),ANCHOR_TOP , s__uiImage_ui[s__toastPop_icon[this]] , ANCHOR_BOTTOM , 0 , - 0.007 / resizeX),ANCHOR_BOTTOM , s__uiImage_ui[s__toastPop_bg[this]] , ANCHOR_BOTTOM , 0 , 0.005),ANCHOR_LEFT , s__uiImage_ui[s__toastPop_bg[this]] , ANCHOR_LEFT , 0.01 , 0),ANCHOR_RIGHT , s__uiImage_ui[s__toastPop_bg[this]] , ANCHOR_RIGHT , - 0.01 , 0),contentText) // 创建内容文本
            return sc__toastPop_initAnimation(this,0.3 , 5.0)
        endfunction  // 创建带标题的弹窗
        function s__toastPop_createWithTitle takes player p,string titleText,string contentText returns integer
            local integer this=0
            local real resizeX=GetResizeRate()
            set this=s__toastPop_initBase(this,p)
            if ( this == 0 ) then // 创建标题文本
                return 0
            endif
            set s__toastPop_title[this]=s__uiText_setPoint(s__uiText_setText(s__uiText_create(s__uiImage_ui[s__toastPop_bg[this]]),titleText),ANCHOR_TOP , s__uiImage_ui[s__toastPop_bg[this]] , ANCHOR_TOP , 0 , - 0.018 / resizeX)
            set s__toastPop_content[this]=s__uiText_setText(s__uiText_setPoint(s__uiText_setPoint(s__uiText_setPoint(s__uiText_setPoint(s__uiText_create(s__uiImage_ui[s__toastPop_bg[this]]),ANCHOR_TOP , s__uiText_ui[s__toastPop_title[this]] , ANCHOR_BOTTOM , 0 , - 0.007 / resizeX),ANCHOR_BOTTOM , s__uiImage_ui[s__toastPop_bg[this]] , ANCHOR_BOTTOM , 0 , 0.005),ANCHOR_LEFT , s__uiImage_ui[s__toastPop_bg[this]] , ANCHOR_LEFT , 0.01 , 0),ANCHOR_RIGHT , s__uiImage_ui[s__toastPop_bg[this]] , ANCHOR_RIGHT , - 0.01 , 0),contentText) // 创建内容文本
            return sc__toastPop_initAnimation(this,0.3 , 5.0)
        endfunction  // 初始化动画相关参数(内部方法)
        function s__toastPop_initAnimation takes integer this,real length,real time returns integer
            set s__toastPop_totalTime[this]=IMaxBJ(R2I(time * 50), 40 + 100 + 40)
            set s__toastPop_cd[this]=s__toastPop_totalTime[this]
            set s__toastPop_length[this]=length // 设置初始状态
            call s__uiText_setAlpha(s__toastPop_content[this],0)
            if ( s__toastPop_title[this] != 0 ) then
                call s__uiText_setAlpha(s__toastPop_title[this],0)
            endif
            if ( s__toastPop_icon[this] != 0 ) then
                call s__uiImage_setAlpha(s__toastPop_icon[this],0)
            endif
            call s__uiImage_setSize(s__toastPop_bg[this],0 , 0) // 加入列表并启动动画
            set s__toastPop_size=s__toastPop_size + 1
            set s__toastPop_List[s__toastPop_size]=this
            set s__toastPop_id[this]=s__toastPop_size
            call s__uianim_reg(s__toastPop_UIA)
            return this
        endfunction
            function s__toastPop_anon__0 takes nothing returns nothing
                local real r
                local real l
                local real resizeX=GetResizeRate()
                local integer i
                ...
                set i=1
                loop
                exitwhen ( i > s__toastPop_size )
                    set this=s__toastPop_List[i]
                    set s__toastPop_cd[this]=s__toastPop_cd[this] - 1 // 阶段1: 弹出动画
                    if ( s__toastPop_cd[this] >= ( s__toastPop_totalTime[this] - 40 ) ) then
                        set r=( I2R(s__toastPop_totalTime[this] - s__toastPop_cd[this]) ) / 40
                        set l=EaseOutBack(r) * s__toastPop_length[this]
                        call s__uiImage_setSize(s__toastPop_bg[this],l , l / resizeX * 0.29032)
                        if ( s__toastPop_title[this] != 0 ) then
                            call s__uiText_setAlpha(s__toastPop_title[this],R2I(255 * EaseInExpo(r)))
                        endif
                        if ( s__toastPop_icon[this] != 0 ) then
                            call s__uiImage_setAlpha(s__toastPop_icon[this],R2I(255 * EaseInExpo(r)))
                        endif
                        if ( s__toastPop_content[this] != 0 ) then
                            call s__uiText_setAlpha(s__toastPop_content[this],R2I(255 * EaseInExpo(r)))
                        endif
                    endif // 阶段3: 消失动画
                    if ( s__toastPop_cd[this] <= 40 ) then
                        set l=( 1.0 - EaseInBack(1.0 - ( I2R(s__toastPop_cd[this]) / 40 )) ) * s__toastPop_length[this]
                        call s__uiImage_setSize(s__toastPop_bg[this],l , l / resizeX * 0.29032)
                        if ( s__toastPop_title[this] != 0 ) then
                            call s__uiText_setAlpha(s__toastPop_title[this],255 - R2I(255 * EaseOutExpo(1.0 - I2R(s__toastPop_cd[this]) / 40)))
                        endif
                        if ( s__toastPop_icon[this] != 0 ) then
                            call s__uiImage_setAlpha(s__toastPop_icon[this],255 - R2I(255 * EaseOutExpo(1.0 - I2R(s__toastPop_cd[this]) / 40)))
                        endif
                        if ( s__toastPop_content[this] != 0 ) then
                            call s__uiText_setAlpha(s__toastPop_content[this],255 - R2I(255 * EaseOutExpo(1.0 - I2R(s__toastPop_cd[this]) / 40)))
                        endif
                    endif // 检查是否结束
                    if ( s__toastPop_cd[this] <= 0 ) then
                        call s__toastPop_deallocate(this)
                        set i=i - 1
                    endif
                set i=i + 1
                endloop
            endfunction
        function s__toastPop_onInit takes nothing returns nothing
            set s__toastPop_UIA=s__uianim_create(function s__toastPop_anon__0)
        endfunction

//library ToastPop ends
//library UTToastPop:

    function UTToastPop___TTestUTToastPop1 takes player p returns nothing
        local integer th=s__toastPop_createWithTitle(p , "这是一个测试提示" , "这是一个测试内容")
    endfunction
    function UTToastPop___TTestUTToastPop2 takes player p returns nothing
        local integer th=s__toastPop_createWithIcon(p , "ReplaceableTextures\\CommandButtons\\BTNRingPurple.blp" , "这是一个测试内容这是一个测试内容这是一个测试内容这是一个测试内容\n\t\t\theheheheheninini")
    endfunction
    function UTToastPop___TTestUTToastPop3 takes player p returns nothing
    endfunction
    function UTToastPop___TTestUTToastPop4 takes player p returns nothing
    endfunction
    function UTToastPop___TTestUTToastPop5 takes player p returns nothing
    endfunction
    function UTToastPop___TTestUTToastPop6 takes player p returns nothing
    endfunction
    function UTToastPop___TTestUTToastPop7 takes player p returns nothing
    endfunction
    function UTToastPop___TTestUTToastPop8 takes player p returns nothing
    endfunction
    function UTToastPop___TTestUTToastPop9 takes player p returns nothing
    endfunction
    function UTToastPop___TTestUTToastPop10 takes player p returns nothing
    endfunction
    function UTToastPop___TTestActUTToastPop1 takes string str returns nothing
        ...
        function UTToastPop___anon__0 takes nothing returns nothing
            call BJDebugMsg("[ToastPop] 单元测试已加载")
            call DestroyTrigger(GetTriggeringTrigger())
        endfunction
        function UTToastPop___anon__1 takes nothing returns nothing
            local string str=GetEventPlayerChatString()
            local integer i=1
            if ( SubStringBJ(str, 1, 1) == "-" ) then
                call UTToastPop___TTestActUTToastPop1(SubStringBJ(str, 2, StringLength(str)))
                return
            endif
            if ( str == "s1" ) then
                call UTToastPop___TTestUTToastPop1(GetTriggerPlayer())
            elseif ( str == "s2" ) then
                call UTToastPop___TTestUTToastPop2(GetTriggerPlayer())
            elseif ( str == "s3" ) then
                call UTToastPop___TTestUTToastPop3(GetTriggerPlayer())
            elseif ( str == "s4" ) then
                call UTToastPop___TTestUTToastPop4(GetTriggerPlayer())
            elseif ( str == "s5" ) then
                call UTToastPop___TTestUTToastPop5(GetTriggerPlayer())
            elseif ( str == "s6" ) then
                call UTToastPop___TTestUTToastPop6(GetTriggerPlayer())
            elseif ( str == "s7" ) then
                call UTToastPop___TTestUTToastPop7(GetTriggerPlayer())
            elseif ( str == "s8" ) then
                call UTToastPop___TTestUTToastPop8(GetTriggerPlayer())
            elseif ( str == "s9" ) then
                call UTToastPop___TTestUTToastPop9(GetTriggerPlayer())
            elseif ( str == "s10" ) then
                call UTToastPop___TTestUTToastPop10(GetTriggerPlayer())
            endif
        endfunction
    function UTToastPop___onInit takes nothing returns nothing
        local trigger tr=CreateTrigger()
        call TriggerRegisterTimerEventSingle(tr, 0.5)
        call TriggerAddCondition(tr, Condition(function UTToastPop___anon__0))
        set tr=null
        call UnitTestRegisterChatEvent(function UTToastPop___anon__1)
    endfunction

//library UTToastPop ends
//窗口的大小
...
call ExecuteFunc("jasshelper__initstructs28968046")
call ExecuteFunc("UnitTestFramwork___onInit")
call ExecuteFunc("UITocInit___onInit")
call ExecuteFunc("UTToastPop___onInit")
...
function sa__toastPop_onDestroy takes nothing returns boolean
...
function sa__toastPop_initAnimation takes nothing returns boolean
local integer this=f__arg_this
local real length=f__arg_real1
local real time=f__arg_real2
            set s__toastPop_totalTime[this]=IMaxBJ(R2I(time * 50), 40 + 100 + 40)
            set s__toastPop_cd[this]=s__toastPop_totalTime[this]
            set s__toastPop_length[this]=length // 设置初始状态
            call s__uiText_setAlpha(s__toastPop_content[this],0)
...
function jasshelper__initstructs28968046 takes nothing returns nothing
    set st__toastPop_onDestroy=CreateTrigger()
    call TriggerAddCondition(st__toastPop_onDestroy,Condition( function sa__toastPop_onDestroy))
    set st__toastPop_initAnimation=CreateTrigger()
    call TriggerAddCondition(st__toastPop_initAnimation,Condition( function sa__toastPop_initAnimation))
    ...
    call ExecuteFunc("s__toastPop_onInit")
endfunction
