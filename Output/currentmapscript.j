...
//library MallItem:
    struct mallItem extends array  // 状态与事件
    //! pragma implicitthis
        private static boolean initialized=false
        private static boolean ready=false
        private static trigger readyTrigger=null  // 数据表
        private static hashtable table=null  // key 映射与临时使用 // 商品列表与映射
        private static integer itemCount=0  // 0..itemCount-1
        private static string  array itemKeys  // 拥有权缓存：owns[player * MALLITEM_MAX_ITEMS + itemIndex] player是从0开始
        private static boolean  array owns  // 使用次数缓存：uses[player * MALLITEM_MAX_ITEMS + itemIndex]
        private static integer  array uses  // 元信息：按 itemIndex 对齐
        private static string  array names
        private static string  array icons
        private static string  array descs  // 科技 ID，如 'R015'（四位字符转换为整数）
        private static integer  array techs  // 回调参数传递（避免哈希表冲突）
        static player callbackPlayer=null  // ========== 内部：解析与映射 =========
        private static method getIndex takes string key returns integer
            local integer stored
            local integer idx
            set stored=LoadInteger(mallItem.table,0,StringHash(key))
            if (stored==0)then
                return -1
            endif  // 存储时 +1，读取时 -1
            set idx=stored-1
            if (idx<0 or idx>=mallItem.itemCount)then
                return -1
            endif
            return idx
        endmethod
        private static method setIndex takes string key,integer index returns nothing
            call SaveInteger(mallItem.table,0,StringHash(key),index+1)
        endmethod
        private static method addKey takes string key returns nothing
            local integer idx
            local integer i
            local integer n
            local integer base
            ...
            set idx=mallItem.getIndex(key)
            if (idx>=0)then
                return
            endif
            if (mallItem.itemCount>=300)then  // 超上限忽略
                return
            endif
            set idx=mallItem.itemCount
            set mallItem.itemKeys[idx]=key
            call mallItem.setIndex(key,idx)  // 默认元信息
            set mallItem.names[idx]=""
            set mallItem.icons[idx]=""
            set mallItem.descs[idx]=""
            set mallItem.techs[idx]=0  // 初始化拥有权为 false（所有玩家）
            set i=0
            loop
            exitwhen (i>=4)
                set base=i*300
                set mallItem.owns[base+idx]=false
                set i=i+1
            endloop
            set mallItem.itemCount=mallItem.itemCount+1
        endmethod  // （移除字符串拆分，改为单商品增量注册）
        static method onInit takes nothing returns nothing  //公用方法 // 初始化底层（在 map 启动时自动调用） // 先声明
            set mallItem.initialized=false  // 无句柄局部变量
            set mallItem.ready=false
            set mallItem.itemCount=0
            set mallItem.table=InitHashtable()
            set mallItem.readyTrigger=CreateTrigger()
        endmethod  // 外部初始化：每次只注册一个商品 key；首次调用时启动延迟扫描
            private static method anon__0 takes nothing returns nothing  // 注册商品（支持多次调用，去重在 addKey 内部完成） // 首次调用时启动延迟扫描 // 延迟初始化玩家商品状态
                local integer pid
                local integer idx
                local integer base
                local player p
                ...
                exitwhen (pid>=4)
                    ...
                    loop
                    exitwhen (idx>=n)
                        set k=mallItem.itemKeys[idx]
                        set mallItem.owns[base+idx]=DzAPI_Map_HasMallItem(p,k)
                        set mallItem.uses[base+idx]=DzAPI_Map_GetMallItemCount(p,k)  // 直接在此处解锁科技（如果拥有商品且设置了科技）
                        if (mallItem.owns[base+idx] and mallItem.techs[idx]!=0)then
                            call SetPlayerTechResearched(p,mallItem.techs[idx],1)
                        endif
                        set idx=idx+1
                    endloop
                    set p=null
                    set pid=pid+1
                endloop
                set mallItem.ready=true
                if (mallItem.readyTrigger!=null)then  // 使用 TriggerEvaluate 调用回调条件
                    call TriggerEvaluate(mallItem.readyTrigger)
                endif
            endmethod  // handler 置空
        static method init takes string productKey returns nothing
            local timer t
            call mallItem.addKey(productKey)
            if (not (mallItem.initialized))then
                set mallItem.initialized=true
                set t=CreateTimer()
                call TimerStart(t,2.0,false,function thistype.anon__0)
                set t=null
            endif
        endmethod  // 是否已完成首次扫描
        static method isReady takes nothing returns boolean
            return mallItem.ready
        endmethod  // 注册 onReady 回调（使用 Condition 封装 code），若已就绪则立即 Evaluate
        static method onReady takes code cb returns nothing
            if (mallItem.readyTrigger==null)then
                set mallItem.readyTrigger=CreateTrigger()
            endif
            call TriggerAddCondition(mallItem.readyTrigger,Condition(cb))
            if (mallItem.ready)then
                call TriggerEvaluate(mallItem.readyTrigger)
            endif
        endmethod  // 拥有权查询：通过玩家句柄
        static method hasByPlayer takes player whichPlayer,string itemKey returns boolean
            local integer pid
            local integer idx
            local integer base
            ...
            set idx=mallItem.getIndex(itemKey)
            if (idx<0)then
                return false
            endif
            set base=pid*300
            set result=mallItem.owns[base+idx]
            return result
        endmethod  // 使用次数查询：通过玩家句柄
        static method getUseCountByPlayer takes player whichPlayer,string itemKey returns integer
            local integer pid
            local integer idx
            local integer base
            set pid=GetPlayerId(whichPlayer)
        ...
        endmethod  // 刷新某玩家的拥有权（对已登记商品）
        static method refreshItemsForPlayer takes integer playerId returns nothing
            local integer i
            local integer base
            local player p
            local string k
            ...
            loop
            exitwhen (i>=n)
                set k=mallItem.itemKeys[i]
                set mallItem.owns[base+i]=DzAPI_Map_HasMallItem(p,k)
                set i=i+1
            endloop
            set p=null
        endmethod  // 消费次数型道具（带回调）：成功消费后调用回调并传入玩家参数
        static method consumeTimes takes player whichPlayer,string itemKey,integer count,code callback returns boolean
            local integer pid
            local integer idx
            local integer base
            local boolean ok
            ...
            if (ok)then
                ...
                set mallItem.owns[base+idx]=DzAPI_Map_HasMallItem(whichPlayer,itemKey)
                set mallItem.uses[base+idx]=DzAPI_Map_GetMallItemCount(whichPlayer,itemKey)  // 如果使用次数小于等于0，则认为该玩家没有这个道具了
                if (mallItem.uses[base+idx]<=0)then
                    set mallItem.owns[base+idx]=false
                endif  // 调用回调（传入玩家参数）
                if (callback!=null)then
                    set mallItem.callbackPlayer=whichPlayer
                    ...
                    set mallItem.callbackPlayer=null
                    set tempTr=null
                endif
            endif
            return ok
        endmethod  // 消费一次性道具（UseConsumablesItem）：无回调
        static method consumeOnce takes player whichPlayer,string itemKey returns nothing
            local integer pid
            local integer idx
            local integer base
            set pid=GetPlayerId(whichPlayer)
        ...
        endmethod  // ========== 元信息写接口 ==========
        static method setName takes string key,string name returns nothing
            local integer idx
            set idx=mallItem.getIndex(key)
            if (idx<0)then
                return
            endif
            set mallItem.names[idx]=name
        endmethod
        static method setIcon takes string key,string iconPath returns nothing
            local integer idx
            set idx=mallItem.getIndex(key)
            if (idx<0)then
                return
            endif
            set mallItem.icons[idx]=iconPath
        endmethod
        static method setDesc takes string key,string desc returns nothing
            local integer idx
            set idx=mallItem.getIndex(key)
            if (idx<0)then
                return
            endif
            set mallItem.descs[idx]=desc
        endmethod
        static method setTech takes string key,integer techId returns nothing
            local integer idx
            set idx=mallItem.getIndex(key)
            if (idx<0)then
                return
            endif
            set mallItem.techs[idx]=techId
        endmethod
        static method setMeta takes string key,string name,string iconPath,string desc returns nothing
            local integer idx
            set idx=mallItem.getIndex(key)
            if (idx<0)then
                return
            ...
            set mallItem.descs[idx]=desc
        endmethod
        static method setMetaWithTech takes string key,string name,string iconPath,string desc,integer techId returns nothing
            local integer idx
            set idx=mallItem.getIndex(key)
            if (idx<0)then
                return
            ...
            set mallItem.techs[idx]=techId
        endmethod  // ========== 元信息读接口 ==========
        static method getName takes string key returns string
            local integer idx
            set idx=mallItem.getIndex(key)
            if (idx<0)then
                return ""
        ...
        endmethod
        static method getIcon takes string key returns string
            local integer idx
            set idx=mallItem.getIndex(key)
            if (idx<0)then
                return ""
        ...
        endmethod
        static method getDesc takes string key returns string
            local integer idx
            set idx=mallItem.getIndex(key)
            if (idx<0)then
                return ""
        ...
        endmethod
        static method getTech takes string key returns integer
            local integer idx
            set idx=mallItem.getIndex(key)
            if (idx<0)then
                return 0
        ...
        endmethod  // 在 consumeTimes 回调中获取触发的玩家
        static method getCallbackPlayer takes nothing returns player
            return mallItem.callbackPlayer
        endmethod
        static method hasItemKey takes string key returns boolean
            return mallItem.getIndex(key)>=0
        endmethod
        static method getAllItemKeys takes nothing returns string
            local integer i
            local string out
            local integer n
            set n=mallItem.itemCount
            ...
            exitwhen (i>=n)
                if (i==0)then
                    set out=mallItem.itemKeys[i]
                else
                    set out=out+","+mallItem.itemKeys[i]
                endif
                set i=i+1
            endloop
            return out
        endmethod  // 根据索引获取商品 key（1-based 外部语义：1 表示第一个）
        static method getItemKeyByIndex takes integer oneBasedIndex returns string
            local integer idx
        ...
        endmethod  // 获取已登记商品数量
        static method getItemCount takes nothing returns integer
...
//library UTMallItem:
    ...
    function UTMallItem__DumpState takes player p returns nothing
        ...
        loop
        exitwhen (i>n)
            set key=mallItem.getItemKeyByIndex(i)
            set has=mallItem.hasByPlayer(Player(pid),key)
            set cnt=mallItem.getUseCountById(pid,key)
            call BJDebugMsg("  - ["+I2S(i)+"] key="+key+", has="+I2S(I3(has,1,0))+", cnt="+I2S(cnt))
        set i = i+1
        endloop
    endfunction
            function UTMallItem__anon__1 takes nothing returns boolean  // 注册测试商品（逐个注册） // 配置元信息与科技 // 就绪后校验 // 次数型消费（带回调）
                local player cbp=mallItem.getCallbackPlayer()
                call BJDebugMsg("[UTMallItem] consumeTimes callback player="+GetPlayerName(cbp))
                call BJDebugMsg("  VIP1 after consume cnt="+I2S(mallItem.getUseCountById(0,"VIP1")))
                return true
            endfunction  // 局数型消费（无回调）
        function UTMallItem__anon__0 takes nothing returns boolean
            local player p0=Player(0)
            call BJDebugMsg("[UTMallItem] onReady reached")
            call UTMallItem__DumpState(p0)
            if (mallItem.hasByPlayer(Player(0),"RhdeKey"))then
                call BJDebugMsg("  Rhde tech count="+I2S(GetPlayerTechCount(p0,'Rhde',true)))
            endif
            if (mallItem.hasByPlayer(Player(0),"RopgKey"))then
                call BJDebugMsg("  Ropg tech count="+I2S(GetPlayerTechCount(p0,'Ropg',true)))
            endif
            call mallItem.consumeTimes(p0,"VIP1",1,function UTMallItem__anon__1)
            call mallItem.consumeOnce(p0,"VIP1")
            set p0=null
            return true
        endfunction  // 演示定时器
        function UTMallItem__anon__2 takes nothing returns nothing  // start: 0.1 秒后
    ...
    function UTMallItem__Init takes nothing returns nothing
        call mallItem.init("VIP1")
        call mallItem.init("RhdeKey")
        call mallItem.init("RopgKey")
        call mallItem.setMeta("VIP1","白金VIP","ReplaceableTextures\\CommandButtons\\BTN.tga","尊享特权")
        call mallItem.setTech("RhdeKey",'Rhde')
        call mallItem.setTech("RopgKey",'Ropg')
        call mallItem.onReady(function UTMallItem__anon__0)
        call UnitTestAutoTimer(0.1,2.0,function UTMallItem__anon__2,function UTMallItem__anon__3)
    endfunction
    function UTMallItem__TTestUTMallItem1 takes player p returns nothing  // mallItem
    endfunction
    ...
    function UTMallItem__TTestUTMallItem10 takes player p returns nothing
        ...
        function UTMallItem__anon__4 takes nothing returns boolean  //获取范围式数字 //所有参数S //所有参数I //所有参数R // ct <key> <count>
            local player cbp=mallItem.getCallbackPlayer()
    ...
    function UTMallItem__TTestActUTMallItem1 takes string str returns nothing
        ...
        set num=num+1
        if (paramS[0]=="mi")then
            call UTMallItem__DumpState(p)
        elseif (paramS[0]=="ct")then
            if (num>=3)then
                call mallItem.consumeTimes(p,paramS[1],paramI[2],function UTMallItem__anon__4)
            else
                call BJDebugMsg("usage: -ct <key> <count>")
            endif
        elseif (paramS[0]=="co")then  // co <key>
            if (num>=2)then
                call mallItem.consumeOnce(p,paramS[1])
        ...
        elseif (paramS[0]=="rf")then  // rf <pid0-based>
            if (num>=2)then
                call mallItem.refreshItemsForPlayer(paramI[1])
        ...
        function UTMallItem__anon__5 takes nothing returns nothing  //在游戏开始0.0秒后再调用
            call BJDebugMsg("[MallItem] 单元测试已加载")
            call UTMallItem__Init()
            call DestroyTrigger(GetTriggeringTrigger())
        endfunction
        function UTMallItem__anon__6 takes nothing returns nothing
    ...
    function UTMallItem__onInit takes nothing returns nothing
        ...
        call TriggerAddCondition(tr,Condition(function UTMallItem__anon__5))
        set tr=null
        call UnitTestRegisterChatEvent(function UTMallItem__anon__6)
    endfunction
...
//        if (mallItem.hasById(0, "VIP1")) {
//            BJDebugMsg("[MallItem] 玩家0拥有VIP1, 次数=" + I2S(mallItem.getUseCountById(0, "VIP1")));
//        }
//        return true;
//    });
//
