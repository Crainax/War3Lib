...
integer UTUIExtendDrag__btn=0
integer UTUIExtendDrag__img=0
...
real s__uiDragger_CENTER_X=0.4
real s__uiDragger_CENTER_Y=0.3
integer array s__uiDragger_frame
real array s__uiDragger_xPos
real array s__uiDragger_yPos
real array s__uiDragger_limitWidth
real array s__uiDragger_limitHeight
...
function sc__uiDragger_create takes integer frame,real width,real height returns integer
...
//Implemented from module extendDrag:
        ...
        function s__uiBtn_enableDrag takes integer this,real width,real height returns nothing
            local integer dragger
            if ( not ( s__uiBtn_isExist(this) ) ) then
                return
            endif
            if ( HaveSavedInteger(HASH_UI, this, 1944) ) then
                set dragger=LoadInteger(HASH_UI, this, 1944)
                set s__uiDragger_frame[dragger]=this
                set s__uiDragger_limitWidth[dragger]=width * 0.5
                set s__uiDragger_limitHeight[dragger]=height * 0.5
            else
                set dragger=sc__uiDragger_create(this , width , height)
                call SaveInteger(HASH_UI, this, 1944, dragger)
            endif
...
//library UIExtendDrag:
        function s__uiDragger_setPosition takes integer this,real newX,real newY returns nothing
            set newX=RLimit(newX , s__uiDragger_CENTER_X - s__uiDragger_limitWidth[this] , s__uiDragger_CENTER_X + s__uiDragger_limitWidth[this])
            set newY=RLimit(newY , s__uiDragger_CENTER_Y - s__uiDragger_limitHeight[this] , s__uiDragger_CENTER_Y + s__uiDragger_limitHeight[this])
            set s__uiDragger_xPos[this]=newX
            set s__uiDragger_yPos[this]=newY
            call DzFrameSetAbsolutePoint(s__uiDragger_frame[this], 4, newX, newY)
        endfunction
        function s__uiDragger_create takes integer frame,real width,real height returns integer
            local integer this=s__uiDragger__allocate()
            set s__uiDragger_frame[this]=frame // 存储活动范围（使用一半的宽高值）
            set s__uiDragger_limitWidth[this]=width * 0.5
            set s__uiDragger_limitHeight[this]=height * 0.5 // 设置初始位置在中心点
            call s__uiDragger_setPosition(this,s__uiDragger_CENTER_X , s__uiDragger_CENTER_Y)
...
function s__uiDragger_deallocate takes integer this returns nothing
    ...
    set si__uiDragger_F=this
            ...
            function s__uiDragger_anon__0 takes nothing returns nothing
                ...
                if ( frame != 0 ) then // 检查frame是否还存在（如果UI被删除，DzGetMouseFocus会返回0）
                    if ( DzGetMouseFocus() == frame ) then
                        set s__uiDragger_draggingFrame=0
                        return
                    endif
                    ...
                    else // 如果找不到dragger实例，也停止拖拽
                        set s__uiDragger_draggingFrame=0
                    endif // 更新鼠标起始位置，避免累积误差
                    set s__uiDragger_startX=s__hardware_getMouseX()
                    set s__uiDragger_startY=s__hardware_getMouseY()
                endif
...
//library UTUIExtendDrag:
    ...
    function UTUIExtendDrag__TTestUTUIExtendDrag1 takes player p returns nothing
        set UTUIExtendDrag__img=s__uiImage_setTexture(s__uiImage_setPoint(s__uiImage_setSize(s__uiImage_create(DzGetGameUI()),0.1 , 0.1),4 , DzGetGameUI() , 4 , 0.0 , 0.0),"ReplaceableTextures\\CommandButtons\\BTNHealOn.blp")
        set UTUIExtendDrag__btn=s__uiBtn_enableDrag(s__uiBtn_setSize(s__uiBtn_create(s__uiImage_ui[UTUIExtendDrag__img]),0.04 , 0.04),0.35 , 0.25)
        call s__UIHashTable__uiHTEvent_bind(s__UIHashTable__uiHT_eventdata[uiHashTable(s__uiBtn_ui[UTUIExtendDrag__btn])],8174) // .setPoint(ANCHOR_CENTER, DzGetGameUI(), ANCHOR_CENTER, 0.0, 0.0);
    endfunction
    function UTUIExtendDrag__TTestUTUIExtendDrag2 takes player p returns nothing
    endfunction
    function UTUIExtendDrag__TTestUTUIExtendDrag3 takes player p returns nothing
    endfunction
    function UTUIExtendDrag__TTestUTUIExtendDrag4 takes player p returns nothing
    endfunction
    function UTUIExtendDrag__TTestUTUIExtendDrag5 takes player p returns nothing
    endfunction
    function UTUIExtendDrag__TTestUTUIExtendDrag6 takes player p returns nothing
    endfunction
    function UTUIExtendDrag__TTestUTUIExtendDrag7 takes player p returns nothing
    endfunction
    function UTUIExtendDrag__TTestUTUIExtendDrag8 takes player p returns nothing
    endfunction
    function UTUIExtendDrag__TTestUTUIExtendDrag9 takes player p returns nothing
    endfunction
    function UTUIExtendDrag__TTestUTUIExtendDrag10 takes player p returns nothing
    endfunction
    function UTUIExtendDrag__TTestActUTUIExtendDrag1 takes string str returns nothing
        ...
        function UTUIExtendDrag__anon__0 takes nothing returns nothing
            call BJDebugMsg("[UIExtendDrag] 单元测试已加载")
            call DestroyTrigger(GetTriggeringTrigger())
        endfunction
        function UTUIExtendDrag__anon__1 takes nothing returns nothing
            local string str=GetEventPlayerChatString()
            local integer i=1
            if ( SubStringBJ(str, 1, 1) == "-" ) then
                call UTUIExtendDrag__TTestActUTUIExtendDrag1(SubStringBJ(str, 2, StringLength(str)))
                return
            endif
            if ( str == "s1" ) then
                call UTUIExtendDrag__TTestUTUIExtendDrag1(GetTriggerPlayer())
            elseif ( str == "s2" ) then
                call UTUIExtendDrag__TTestUTUIExtendDrag2(GetTriggerPlayer())
            elseif ( str == "s3" ) then
                call UTUIExtendDrag__TTestUTUIExtendDrag3(GetTriggerPlayer())
            elseif ( str == "s4" ) then
                call UTUIExtendDrag__TTestUTUIExtendDrag4(GetTriggerPlayer())
            elseif ( str == "s5" ) then
                call UTUIExtendDrag__TTestUTUIExtendDrag5(GetTriggerPlayer())
            elseif ( str == "s6" ) then
                call UTUIExtendDrag__TTestUTUIExtendDrag6(GetTriggerPlayer())
            elseif ( str == "s7" ) then
                call UTUIExtendDrag__TTestUTUIExtendDrag7(GetTriggerPlayer())
            elseif ( str == "s8" ) then
                call UTUIExtendDrag__TTestUTUIExtendDrag8(GetTriggerPlayer())
            elseif ( str == "s9" ) then
                call UTUIExtendDrag__TTestUTUIExtendDrag9(GetTriggerPlayer())
            elseif ( str == "s10" ) then
                call UTUIExtendDrag__TTestUTUIExtendDrag10(GetTriggerPlayer())
            endif
        endfunction
    function UTUIExtendDrag__onInit takes nothing returns nothing
        local trigger tr=CreateTrigger()
        call TriggerRegisterTimerEventSingle(tr, 0.5)
        call TriggerAddCondition(tr, Condition(function UTUIExtendDrag__anon__0))
        set tr=null
        call UnitTestRegisterChatEvent(function UTUIExtendDrag__anon__1)
    endfunction
...
call ExecuteFunc("jasshelper__initstructs25377109")
...
call ExecuteFunc("UTUIExtendDrag__onInit")
...
function jasshelper__initstructs25377109 takes nothing returns nothing
