...
integer UTUnitSpell___simpleSpellInitCount=0
integer UTUnitSpell___simpleSpellDestroyCount=0
integer UTUnitSpell___spellDataInitCount=0
integer UTUnitSpell___spellDataDestroyCount=0
integer UTUnitSpell___spellDestroyCount=0
integer UTUnitSpell___unitDataInitCount=0
integer UTUnitSpell___destroyCount=0
...
unit s__spellData_argsU=null
integer s__spellData_argsLevel=0
...
integer array s__unitSpell_simpleCount
...
function s__unitSpell__allocate takes nothing returns integer
 local integer this=si__unitSpell_F
   ...
   set s__unitSpell_simpleCount[this]=0
...
//library Spell:
    function GetHashValue takes integer handleID,integer customId returns integer
        ...
        function s__spell_entity takes unit u,integer id,integer level returns integer
            ...
            if ( s__spellData_trInit[s__spell_sd[this]] != null ) then
                set s__spellData_argsU=s__spell_u[this]
                set s__spellData_argsLevel=level
                set s__spell_ethis=this
                call TriggerEvaluate(s__spellData_trInit[s__spell_sd[this]])
            endif
            return this
        ...
        function s__spell_mirror takes unit u,integer id,integer sd,integer level returns integer
            ...
            set s__spell_level[this]=level
            call SaveInteger(HASH_SPELL, key, 15, this)
            if ( s__spellData_trInit[sd] != null ) then
                set s__spellData_argsU=s__spell_u[this]
                set s__spellData_argsLevel=level
                set s__spell_ethis=this
                call TriggerEvaluate(s__spellData_trInit[sd])
            endif
            return this
        ...
        function s__spell_onDestroy takes integer this returns nothing
            if ( not ( s__spell_isExist(this) ) ) then //spell的销毁回调
                return
            endif
            if ( s__spell_trDestroy[this] != null ) then
                call Trace("调用spell的销毁回调")
                set s__spell_ethis=this
                call TriggerEvaluate(s__spell_trDestroy[this])
                call DestroyTrigger(s__spell_trDestroy[this])
                set s__spell_trDestroy[this]=null
            endif //spellData的销毁回调
            if ( s__spellData_trDestroy[s__spell_sd[this]] != null ) then
                call Trace("调用spellData的销毁回调, id=" + I2S(s__spell_id[this]))
                set s__spellData_argsU=s__spell_u[this]
                set s__spellData_argsLevel=s__spell_level[this]
                call TriggerEvaluate(s__spellData_trDestroy[s__spell_sd[this]])
            endif //虚拟技能
...
//library Spell ends
//library SpellData:
        function s__spellData_registerInit takes integer this,code func returns nothing
            if ( s__spellData_trInit[this] == null ) then
                set s__spellData_trInit[this]=CreateTrigger()
            endif
            call TriggerAddCondition(s__spellData_trInit[this], Condition(func))
        endfunction
        function s__spellData_registerDestroy takes integer this,code func returns nothing
            if ( s__spellData_trDestroy[this] == null ) then
                set s__spellData_trDestroy[this]=CreateTrigger()
            endif
            call TriggerAddCondition(s__spellData_trDestroy[this], Condition(func))
        endfunction
        function s__spellData_registerUpgrade takes integer this,code func returns nothing
            if ( s__spellData_trUpgrade[this] == null ) then
                set s__spellData_trUpgrade[this]=CreateTrigger()
            endif
            call TriggerAddCondition(s__spellData_trUpgrade[this], Condition(func))
        endfunction  //根据技能类型
        function s__spellData_byType takes integer at returns integer
            ...
            return this
        endfunction  // 返回一个新的并自增(空物编)
        function s__spellData_new takes nothing returns integer
            local integer this
            set s__spellData_counter=s__spellData_counter + 1
            set this=(s__spellData_counter)
            return this
...
//library UnitSpell:
        ...
        function s__unitSpell_hasSpell takes integer this,integer sp returns boolean
            ...
            endloop
            return false
        endfunction  // 检查单位是否拥有指定技能类型(含简单技能)
        function s__unitSpell_hasSpellData takes integer this,integer sd returns boolean
            local integer i
            local integer handleId=GetHandleId(s__unitSpell_u[this])
            local integer sp
            local integer existingSpell
            if ( s__spellData_spellType[sd] == SPELL_TYPE_SIMPLE ) then
                set i=0
                loop
                exitwhen ( i >= s__unitSpell_simpleCount[this] )
                    if ( HaveSavedInteger(HASH_UNIT, handleId, 2000 + i) ) then
                        set existingSpell=LoadInteger(HASH_UNIT, handleId, 2000 + i) // 检查是否是同一个spellData
                        if ( existingSpell == sd ) then
                            return true
                        endif
                    endif
                set i=i + 1
                endloop
                return false //其他技能的判断
            else
                set i=0
                loop
                exitwhen ( i >= s__unitSpell_spellCount[this] )
                    set sp=LoadInteger(HASH_UNIT, handleId, 1800 + i)
                    if ( s__spell_sd[sp] == sd ) then
                        return true
                    endif
                set i=i + 1
                endloop
        ...
        function s__unitSpell_addSpellData takes integer this,integer sd,integer lv returns boolean
            local integer sp=0
            local integer level=IMinBJ(lv, IMaxBJ(s__spellData_maxLevel[sd], 1))
            if ( s__spellData_spellType[sd] == SPELL_TYPE_SIMPLE ) then
                if ( s__unitSpell_simpleCount[this] >= 200 or s__unitSpell_hasSpellData(this,sd) ) then
                    return false
                endif
                if ( s__spellData_trInit[sd] != null ) then
                    set s__spellData_argsU=s__unitSpell_u[this]
                    set s__spellData_argsLevel=level
                    call TriggerEvaluate(s__spellData_trInit[sd])
                endif
                call SaveInteger(HASH_UNIT, GetHandleId(s__unitSpell_u[this]), 2000 + s__unitSpell_simpleCount[this], sd)
                call SaveInteger(HASH_UNIT, GetHandleId(s__unitSpell_u[this]), 2200 + s__unitSpell_simpleCount[this], level) // 保存技能等级
                set s__unitSpell_simpleCount[this]=s__unitSpell_simpleCount[this] + 1
                return true
            endif //以下是其他技能
            ...
            if ( s__spellData_spellType[sd] == SPELL_TYPE_ENTITY ) then
                set sp=s__spell_entity(s__unitSpell_u[this] , s__spellData_id[sd] , level) // } else if (sd.spellType == SPELL_TYPE_MIRROR) {
            elseif ( s__spellData_spellType[sd] == SPELL_TYPE_VIRTUAL ) then //     sp = spell.mirror(this.u, sd.id, level);
                set sp=s__spell_virtual(s__unitSpell_u[this] , s__spellData_id[sd] , level)
            endif
            if ( sp == 0 ) then
                return false
        ...
        function s__unitSpell_addSpell takes integer this,integer sp returns boolean
            if ( s__unitSpell_spellCount[this] >= 200 ) then
                return false
            endif
        ...
        endfunction  // 获取简单技能数量
        function s__unitSpell_getSimpleSpellCount takes integer this returns integer
            return s__unitSpell_simpleCount[this]
        endfunction  // 获取指定索引的技能(从0开始到count-1)
        function s__unitSpell_getSpell takes integer this,integer index returns integer
            ...
            return 0
        endfunction  // 获取指定索引的简单技能类型(从0开始到count-1)
        function s__unitSpell_getSimpleSpell takes integer this,integer index returns integer
            local integer handleId=GetHandleId(s__unitSpell_u[this])
            local integer sd
            if ( index >= 0 and index < s__unitSpell_simpleCount[this] ) then
                set sd=LoadInteger(HASH_UNIT, handleId, 2000 + index)
                return sd
            endif
            return 0
        endfunction  // 获取指定Simple技能的等级
        function s__unitSpell_getSimpleSpellLevel takes integer this,integer sd returns integer
            local integer i=0
            local integer handleId=GetHandleId(s__unitSpell_u[this])
            local integer targetSpell
            set i=0
            loop
            exitwhen ( i >= s__unitSpell_simpleCount[this] )
                set targetSpell=LoadInteger(HASH_UNIT, handleId, 2000 + i)
                if ( targetSpell == sd ) then
                    return LoadInteger(HASH_UNIT, handleId, 2200 + i)
                endif
            set i=i + 1
            endloop // 未找到该技能返回0
            return 0
        ...
        function s__unitSpell_removeSpell takes integer this,integer sp returns boolean
            ...
            endloop
            return false
        endfunction  // 移除指定的简单技能
        function s__unitSpell_removeSimpleSpell takes integer this,integer sd returns boolean
            local integer i=0
            local integer handleId=GetHandleId(s__unitSpell_u[this])
            local integer lastSpell=0
            local integer targetSpell=0
            local integer targetLevel=0
            local integer lastLevel=0
            set i=0
            loop
            exitwhen ( i >= s__unitSpell_simpleCount[this] )
                set targetSpell=LoadInteger(HASH_UNIT, handleId, 2000 + i)
                if ( targetSpell == sd ) then // 获取技能等级并调用销毁触发器
                    set targetLevel=LoadInteger(HASH_UNIT, handleId, 2200 + i)
                    if ( s__spellData_trDestroy[sd] != null ) then
                        set s__spellData_argsU=s__unitSpell_u[this]
                        set s__spellData_argsLevel=targetLevel
                        call TriggerEvaluate(s__spellData_trDestroy[sd])
                    endif // 如果不是最后一个技能,则把最后一个技能移到当前位置
                    if ( i < s__unitSpell_simpleCount[this] - 1 ) then
                        set lastSpell=LoadInteger(HASH_UNIT, handleId, 2000 + s__unitSpell_simpleCount[this] - 1)
                        set lastLevel=LoadInteger(HASH_UNIT, handleId, 2200 + s__unitSpell_simpleCount[this] - 1)
                        call SaveInteger(HASH_UNIT, handleId, 2000 + i, lastSpell)
                        call SaveInteger(HASH_UNIT, handleId, 2200 + i, lastLevel)
                    endif // 清理最后一个位置
                    call RemoveSavedInteger(HASH_UNIT, handleId, 2000 + s__unitSpell_simpleCount[this] - 1)
                    call RemoveSavedInteger(HASH_UNIT, handleId, 2200 + s__unitSpell_simpleCount[this] - 1)
                    set s__unitSpell_simpleCount[this]=s__unitSpell_simpleCount[this] - 1
                    return true
                endif
            set i=i + 1
        ...
        function s__unitSpell_removeSpellData takes integer this,integer sd returns boolean
            local integer sp
            if ( s__spellData_spellType[sd] == SPELL_TYPE_SIMPLE ) then
                return s__unitSpell_removeSimpleSpell(this,sd)
            else
                set sp=s__spell_get(s__unitSpell_u[this] , s__spellData_id[sd])
                if ( sp != 0 ) then
        ...
        function s__unitSpell_initDefaultSpell takes integer this returns nothing
            ...
            set s__unitSpell_spellCount[this]=0 // 初始化简单技能数量
            set s__unitSpell_simpleCount[this]=0 // 从unitData创建所有技能
            set i=0
            loop
        ...
        function s__unitSpell_onDestroy takes integer this returns nothing
            ...
            exitwhen ( i >= s__unitSpell_spellCount[this] )
                set sp=LoadInteger(HASH_UNIT, handleId, 1800 + i)
                if ( sp != 0 ) then
                    call Trace("销毁Entity技能 #" + I2S(i))
                    call s__spell_deallocate(sp)
                endif
                call RemoveSavedInteger(HASH_UNIT, handleId, 1800 + i)
            set i=i + 1
            endloop // 清理简单技能的引用和调用销毁触发器
            set i=0
            loop
            exitwhen ( i >= s__unitSpell_simpleCount[this] )
                set sd=LoadInteger(HASH_UNIT, handleId, 2000 + i)
                if ( sd != 0 and s__spellData_trDestroy[sd] != null ) then
                    call Trace("销毁Simple技能 #" + I2S(i))
                    set level=LoadInteger(HASH_UNIT, handleId, 2200 + i)
                    set s__spellData_argsU=s__unitSpell_u[this]
                    set s__spellData_argsLevel=level
                    call TriggerEvaluate(s__spellData_trDestroy[sd])
                endif
                call RemoveSavedInteger(HASH_UNIT, handleId, 2000 + i)
                call RemoveSavedInteger(HASH_UNIT, handleId, 2200 + i)
            set i=i + 1
            endloop // 清理自身的引用
            if ( HaveSavedInteger(HASH_UNIT, handleId, 1730) ) then
                call RemoveSavedInteger(HASH_UNIT, handleId, 1730)
            endif
...
//library UTUnitSpell:
        ...
        endfunction  // 测试3: addSpell和getSpell - 0.3秒
        ...
        endfunction  // 测试4: getSpellCount - 0.4秒
        ...
        function UTUnitSpell___anon__5 takes nothing returns nothing
            ...
            exitwhen ( i >= 5 )
                ...
                call s__unitSpell_addSpellData(UTUnitSpell___us,sd , 1)
            set i=i + 1
            endloop // 测试技能数量
            ...
            exitwhen ( i >= 5 )
                set sd=s__spellData_byType(spellIds[i])
                set removeResult=s__unitSpell_removeSpellData(UTUnitSpell___us,sd)
        ...
        function UTUnitSpell___anon__6 takes nothing returns nothing
            ...
            call s__assert_Boolean(s__unitSpell_addSpell(UTUnitSpell___us,sp1) , "首次添加技能实例应该成功")
            call s__assert_Boolean(not ( s__unitSpell_addSpell(UTUnitSpell___us,sp1) ) , "重复添加相同技能实例应该失败")
            ...
            call s__assert_Boolean(s__unitSpell_addSpellData(UTUnitSpell___us,sd , 1) , "首次通过spellData添加技能应该成功")
            call s__assert_Boolean(not ( s__unitSpell_addSpellData(UTUnitSpell___us,sd , 1) ) , "重复添加相同spellData应该失败")
            call s__assert_Integer(s__unitSpell_getSpellCount(UTUnitSpell___us) , 2 , "重复添加后技能数量应该为2")
        endfunction  // 测试9: Simple技能测试 - 0.8秒
            function UTUnitSpell___anon__8 takes nothing returns nothing
                set UTUnitSpell___simpleSpellInitCount=UTUnitSpell___simpleSpellInitCount + 1
                call Trace("Simple技能初始化回调被调用, 当前计数: " + I2S(UTUnitSpell___simpleSpellInitCount))
            endfunction
            function UTUnitSpell___anon__9 takes nothing returns nothing
                set UTUnitSpell___simpleSpellDestroyCount=UTUnitSpell___simpleSpellDestroyCount + 1
                call Trace("Simple技能销毁回调被调用, 当前计数: " + I2S(UTUnitSpell___simpleSpellDestroyCount))
            endfunction  // 测试添加Simple技能
        function UTUnitSpell___anon__7 takes nothing returns nothing
            local integer sd
            local integer array sds
            local integer i
            local boolean addResult=false
            local boolean removeResult=false
            call Trace("开始测试9: Simple技能测试")
            if ( UTUnitSpell___testUnit != null ) then
                call h__RemoveUnit(UTUnitSpell___testUnit)
            endif
            set UTUnitSpell___testUnit=CreateUnit(Player(0), 'hfoo', 0, 0, 0)
            set UTUnitSpell___us=s__unitSpell_parse(UTUnitSpell___testUnit)
            set sd=s__spellData_new()
            set s__spellData_spellType[sd]=SPELL_TYPE_SIMPLE
            set s__spellData_maxLevel[sd]=3
            set i=1
            loop
            exitwhen ( i > 3 )
                set sds[i]=s__spellData_new()
                set s__spellData_spellType[sds[i]]=SPELL_TYPE_SIMPLE
                set s__spellData_maxLevel[sds[i]]=2 + i
            set i=i + 1
            endloop
            call s__spellData_registerInit(sd,function UTUnitSpell___anon__8)
            call s__spellData_registerDestroy(sd,function UTUnitSpell___anon__9)
            set addResult=s__unitSpell_addSpellData(UTUnitSpell___us,sd , 2)
            call s__assert_Boolean(addResult , "添加Simple技能应该成功")
            call s__assert_Integer(s__unitSpell_getSimpleSpellCount(UTUnitSpell___us) , 1 , "Simple技能数量应该为1")
            call s__assert_Integer(UTUnitSpell___simpleSpellInitCount , 1 , "Simple技能初始化回调应该被调用一次") // 测试获取Simple技能
            call s__assert_Boolean(s__unitSpell_getSimpleSpell(UTUnitSpell___us,0) == sd , "获取Simple技能应该返回正确的spellData")
            call s__assert_Boolean(s__unitSpell_hasSpellData(UTUnitSpell___us,sd) , "hasSpellData应该能检测到Simple技能") // 测试重复添加
            set addResult=s__unitSpell_addSpellData(UTUnitSpell___us,sd , 2)
            call s__assert_Boolean(not addResult , "重复添加Simple技能应该失败")
            call s__assert_Integer(s__unitSpell_getSimpleSpellCount(UTUnitSpell___us) , 1 , "重复添加后Simple技能数量应该仍为1") // 测试删除Simple技能
            set removeResult=s__unitSpell_removeSpellData(UTUnitSpell___us,sd)
            call s__assert_Boolean(removeResult , "删除Simple技能应该成功")
            call s__assert_Integer(s__unitSpell_getSimpleSpellCount(UTUnitSpell___us) , 0 , "删除后Simple技能数量应该为0")
            call s__assert_Integer(UTUnitSpell___simpleSpellDestroyCount , 1 , "Simple技能销毁回调应该被调用一次") // 测试删除不存在的Simple技能
            set removeResult=s__unitSpell_removeSpellData(UTUnitSpell___us,sd)
            call s__assert_Boolean(not removeResult , "删除不存在的Simple技能应该失败") // 测试添加多个Simple技能
            set i=1
            loop
            exitwhen ( i > 3 )
                set addResult=s__unitSpell_addSpellData(UTUnitSpell___us,sds[i] , i)
                call s__assert_Boolean(addResult , "添加第" + I2S(i) + "个Simple技能应该成功")
                call s__assert_Integer(s__unitSpell_getSimpleSpellCount(UTUnitSpell___us) , i , "添加" + I2S(i) + "个Simple技能后数量应该为" + I2S(i))
            set i=i + 1
            endloop // 测试获取多个Simple技能
            set i=1
            loop
            exitwhen ( i > 3 )
                call s__assert_Boolean(s__unitSpell_getSimpleSpell(UTUnitSpell___us,i - 1) == sds[i] , "获取第" + I2S(i) + "个Simple技能应该返回正确的spellData")
                call s__assert_Boolean(s__unitSpell_hasSpellData(UTUnitSpell___us,sds[i]) , "hasSpellData应该能检测到第" + I2S(i) + "个Simple技能")
            set i=i + 1
            endloop // 测试逐个删除Simple技能
            set i=1
            loop
            exitwhen ( i > 3 )
                set removeResult=s__unitSpell_removeSpellData(UTUnitSpell___us,sds[i])
                call s__assert_Boolean(removeResult , "删除第" + I2S(i) + "个Simple技能应该成功")
                call s__assert_Integer(s__unitSpell_getSimpleSpellCount(UTUnitSpell___us) , 3 - i , "删除后Simple技能数量应该为" + I2S(3 - i))
            set i=i + 1
            endloop // 测试全部删除后再次删除
            set i=1
            loop
            exitwhen ( i > 3 )
                set removeResult=s__unitSpell_removeSpellData(UTUnitSpell___us,sds[i])
                call s__assert_Boolean(not removeResult , "删除已删除的Simple技能应该失败")
            set i=i + 1
            endloop
        endfunction  // 测试10: 直接删除单位时的技能清理测试
            function UTUnitSpell___anon__11 takes nothing returns nothing
                set UTUnitSpell___destroyCount=UTUnitSpell___destroyCount + 1
            endfunction
            function UTUnitSpell___anon__12 takes nothing returns nothing
                set UTUnitSpell___destroyCount=UTUnitSpell___destroyCount + 10
            endfunction
            function UTUnitSpell___anon__13 takes nothing returns nothing
                set UTUnitSpell___destroyCount=UTUnitSpell___destroyCount + 100
            endfunction  // 创建2个Entity技能
            function UTUnitSpell___anon__14 takes nothing returns nothing
                set UTUnitSpell___destroyCount=UTUnitSpell___destroyCount + 1000
            endfunction
            function UTUnitSpell___anon__15 takes nothing returns nothing
                set UTUnitSpell___destroyCount=UTUnitSpell___destroyCount + 10000
            endfunction  // 添加所有技能
        function UTUnitSpell___anon__10 takes nothing returns nothing
            local integer sd1
            local integer sd2
            local integer sd3
            local integer sd4
            local integer sd5
            call Trace("开始测试10: 直接删除单位时的技能清理测试")
            if ( UTUnitSpell___testUnit != null ) then
                call h__RemoveUnit(UTUnitSpell___testUnit)
            endif
            set UTUnitSpell___destroyCount=0
            set UTUnitSpell___testUnit=CreateUnit(Player(0), 'hfoo', 0, 0, 0)
            set UTUnitSpell___us=s__unitSpell_parse(UTUnitSpell___testUnit)
            set sd1=s__spellData_new()
            set s__spellData_spellType[sd1]=SPELL_TYPE_SIMPLE
            set s__spellData_maxLevel[sd1]=1
            call s__spellData_registerDestroy(sd1,function UTUnitSpell___anon__11)
            set sd2=s__spellData_new()
            set s__spellData_spellType[sd2]=SPELL_TYPE_SIMPLE
            set s__spellData_maxLevel[sd2]=1
            call s__spellData_registerDestroy(sd2,function UTUnitSpell___anon__12)
            set sd3=s__spellData_new()
            set s__spellData_spellType[sd3]=SPELL_TYPE_SIMPLE
            set s__spellData_maxLevel[sd3]=1
            call s__spellData_registerDestroy(sd3,function UTUnitSpell___anon__13)
            set sd4=s__spellData_new()
            set s__spellData_spellType[sd4]=SPELL_TYPE_ENTITY
            set s__spellData_id[sd4]='AHbz'
            set s__spellData_maxLevel[sd4]=1
            call s__spellData_registerDestroy(sd4,function UTUnitSpell___anon__14)
            set sd5=s__spellData_new()
            set s__spellData_spellType[sd5]=SPELL_TYPE_ENTITY
            set s__spellData_id[sd5]='AHtb'
            set s__spellData_maxLevel[sd5]=1
            call s__spellData_registerDestroy(sd5,function UTUnitSpell___anon__15)
            call s__unitSpell_addSpellData(UTUnitSpell___us,sd1 , 1) // +1 // +10
            call s__unitSpell_addSpellData(UTUnitSpell___us,sd2 , 1) // +100
            call s__unitSpell_addSpellData(UTUnitSpell___us,sd3 , 1) // +1000
            call s__unitSpell_addSpellData(UTUnitSpell___us,sd4 , 1) // +10000
            call s__unitSpell_addSpellData(UTUnitSpell___us,sd5 , 1) // 直接删除单位
            call h__RemoveUnit(UTUnitSpell___testUnit)
            set UTUnitSpell___testUnit=null // 验证所有回调都被调用 (1 + 10 + 100 + 1000 + 10000 = 11111)
            call s__assert_Integer(UTUnitSpell___destroyCount , 11111 , "删除单位后所有技能的销毁回调应该被正确调用,值应该为11111")
        endfunction  // 测试11: 技能数量上限测试
        function UTUnitSpell___anon__16 takes nothing returns nothing
            local integer sd
            local integer i
            local boolean addResult
            call Trace("开始测试11: 技能数量上限测试")
            if ( UTUnitSpell___testUnit != null ) then
                call h__RemoveUnit(UTUnitSpell___testUnit)
            endif
            set UTUnitSpell___testUnit=CreateUnit(Player(0), 'hfoo', 0, 0, 0)
            set UTUnitSpell___us=s__unitSpell_parse(UTUnitSpell___testUnit) // 尝试添加201个不同的技能
            set i=0 // 每次创建新的spellData
            loop
            exitwhen ( i >= 201 )
                set sd=s__spellData_new()
                set s__spellData_spellType[sd]=SPELL_TYPE_SIMPLE
                set s__spellData_maxLevel[sd]=1
                set addResult=s__unitSpell_addSpellData(UTUnitSpell___us,sd , 1)
                if ( i < 200 ) then
                    if ( ModuloInteger(i, 10) == 0 ) then
                        call s__assert_Boolean(addResult , "添加第" + I2S(IMaxBJ(i - 9, 0)) + "-" + I2S(i + 1) + "个技能应该成功")
                    endif
                else
                    call s__assert_Boolean(not addResult , "添加第201个技能应该失败")
                endif
            set i=i + 1
            endloop // 验证最终技能数量
            call s__assert_Integer(s__unitSpell_getSimpleSpellCount(UTUnitSpell___us) , 200 , "最终Simple技能数量应该为200")
        endfunction  // 测试12: unitData技能初始化测试
            function UTUnitSpell___anon__18 takes nothing returns nothing
                set UTUnitSpell___unitDataInitCount=UTUnitSpell___unitDataInitCount + 1
                call Trace("spellData1初始化回调被调用")
            endfunction
            function UTUnitSpell___anon__19 takes nothing returns nothing
                set UTUnitSpell___unitDataInitCount=UTUnitSpell___unitDataInitCount + 10
                call Trace("spellData2初始化回调被调用")
            endfunction
            function UTUnitSpell___anon__20 takes nothing returns nothing
                set UTUnitSpell___unitDataInitCount=UTUnitSpell___unitDataInitCount + 100
                call Trace("spellData3初始化回调被调用")
            endfunction  // 获取食尸鬼的unitData并添加技能
        function UTUnitSpell___anon__17 takes nothing returns nothing
            local integer ud
            local integer sd1
            local integer sd2
            local integer sd3
            local unit UTUnitSpell___testUnit
            local integer usl
            set UTUnitSpell___unitDataInitCount=0
            call Trace("开始测试12: unitData技能初始化测试")
            set sd1=s__spellData_new()
            set s__spellData_spellType[sd1]=SPELL_TYPE_SIMPLE
            set s__spellData_maxLevel[sd1]=3
            call s__spellData_registerInit(sd1,function UTUnitSpell___anon__18)
            set sd2=s__spellData_new()
            set s__spellData_spellType[sd2]=SPELL_TYPE_ENTITY
            set s__spellData_id[sd2]='AHbz'
            set s__spellData_maxLevel[sd2]=2
            call s__spellData_registerInit(sd2,function UTUnitSpell___anon__19)
            set sd3=s__spellData_new()
            set s__spellData_spellType[sd3]=SPELL_TYPE_ENTITY
            set s__spellData_id[sd3]='AHtb'
            set s__spellData_maxLevel[sd3]=4
            call s__spellData_registerInit(sd3,function UTUnitSpell___anon__20)
            set ud=s__unitData_byType('ugho') // Simple技能 等级2
            call s__unitData_addSpell(ud,sd1 , 2) // Entity技能 等级1
            call s__unitData_addSpell(ud,sd2 , 1) // Entity技能 等级3
            call s__unitData_addSpell(ud,sd3 , 3) // 验证unitData中的技能数据是否正确
            call s__assert_Integer(s__unitData_getSpellCount(ud) , 3 , "unitData中应该有3个技能")
            call s__assert_Integer(s__unitData_getSpellId(ud,0) , sd1 , "第1个技能应该是sd1:" + I2S(sd1))
            call s__assert_Integer(s__unitData_getSpellId(ud,1) , sd2 , "第2个技能应该是sd2:" + I2S(sd2))
            call s__assert_Integer(s__unitData_getSpellId(ud,2) , sd3 , "第3个技能应该是sd3:" + I2S(sd3))
            call s__assert_Integer(s__unitData_getSpellLevel(ud,0) , 2 , "sd1的等级应该是2")
            call s__assert_Integer(s__unitData_getSpellLevel(ud,1) , 1 , "sd2的等级应该是1")
            call s__assert_Integer(s__unitData_getSpellLevel(ud,2) , 3 , "sd3的等级应该是3") // 创建食尸鬼单位并初始化unitSpell
            set UTUnitSpell___testUnit=CreateUnit(Player(0), 'ugho', 0, 0, 0)
            set usl=s__unitSpell_parse(UTUnitSpell___testUnit) // 验证unitSpell是否正确初始化了所有技能
            call s__assert_Integer(s__unitSpell_getSimpleSpellCount(usl) , 1 , "应该有1个Simple技能")
            call s__assert_Integer(s__unitSpell_getSpellCount(usl) , 2 , "应该有2个Entity技能") // 验证Simple技能
            call s__assert_Boolean(s__unitSpell_getSimpleSpell(usl,0) == sd1 , "Simple技能应该是sd1") // 验证Entity技能
            call s__assert_Boolean(s__spell_sd[s__unitSpell_getSpell(usl,0)] == sd2 , "第1个Entity技能应该是sd2")
            call s__assert_Boolean(s__spell_sd[s__unitSpell_getSpell(usl,1)] == sd3 , "第2个Entity技能应该是sd3") // 验证所有技能的等级
            call s__assert_Integer(s__unitSpell_getSimpleSpellLevel(usl,sd1) , 2 , "Simple技能sd1的等级应该是2")
            call s__assert_Integer(s__spell_level[s__unitSpell_getSpell(usl,0)] , 1 , "sd2的等级应该是1")
            call s__assert_Integer(s__spell_level[s__unitSpell_getSpell(usl,1)] , 3 , "sd3的等级应该是3") // 验证初始化回调是否都被调用 (1 + 10 + 100 = 111)
            call s__assert_Integer(UTUnitSpell___unitDataInitCount , 111 , "所有spellData的初始化回调都应该被调用") // 清理
            call h__RemoveUnit(UTUnitSpell___testUnit)
            set UTUnitSpell___testUnit=null
        endfunction
    function UTUnitSpell___Init takes nothing returns nothing
        ...
        call UnitTestAutoTimer(0.2 , 0 , function UTUnitSpell___anon__1 , null)
        call UnitTestAutoTimer(0.3 , 0 , function UTUnitSpell___anon__2 , null)
        call UnitTestAutoTimer(0.4 , 0 , function UTUnitSpell___anon__3 , null)
        call UnitTestAutoTimer(0.5 , 0 , function UTUnitSpell___anon__4 , null)
        call UnitTestAutoTimer(0.6 , 0 , function UTUnitSpell___anon__5 , null)
        call UnitTestAutoTimer(0.7 , 0 , function UTUnitSpell___anon__6 , null)
        call UnitTestAutoTimer(0.8 , 0 , function UTUnitSpell___anon__7 , null)
        call UnitTestAutoTimer(0.9 , 0 , function UTUnitSpell___anon__10 , null)
        call UnitTestAutoTimer(1.0 , 0 , function UTUnitSpell___anon__16 , null)
        call UnitTestAutoTimer(1.1 , 0 , function UTUnitSpell___anon__17 , null)
    ...
    function UTUnitSpell___TTestUTUnitSpell4 takes player p returns nothing
        ...
        function UTUnitSpell___anon__21 takes nothing returns nothing
    ...
    function UTUnitSpell___TTestUTUnitSpell5 takes player p returns nothing
        ...
        if ( UTUnitSpell___toggle5 ) then
            ...
            call ForGroup(g, function UTUnitSpell___anon__21)
            call DestroyGroup(g)
            set g=null
    ...
    function UTUnitSpell___TTestUTUnitSpell11 takes player p returns nothing
    endfunction
    function UTUnitSpell___TTestActUTUnitSpell1 takes string str returns nothing
        ...
        function UTUnitSpell___anon__22 takes nothing returns nothing
            call Trace("[UnitSpell] 单元测试已加载")
            call UTUnitSpell___Init()
            call DestroyTrigger(GetTriggeringTrigger())
        endfunction
        function UTUnitSpell___anon__23 takes nothing returns nothing
            ...
            elseif ( str == "s11" ) then
                call UTUnitSpell___TTestUTUnitSpell11(GetTriggerPlayer())
            endif
        endfunction
    function UTUnitSpell___onInit takes nothing returns nothing
        local trigger tr=CreateTrigger()
        call TriggerRegisterTimerEvent(tr, 0.5, false)
        call TriggerAddCondition(tr, Condition(function UTUnitSpell___anon__22))
        set tr=null
        call UnitTestRegisterChatEvent(function UTUnitSpell___anon__23)
    endfunction
...
call ExecuteFunc("jasshelper__initstructs3825890")
...
function jasshelper__initstructs3825890 takes nothing returns nothing
