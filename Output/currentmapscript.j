	...
	function TTestUTLogger2(player p) {
		...
		for(0 <= i <= 11) {
			if(GetPlayerController(Player(i)) == MAP_CONTROL_USER) {
				TraceToPlayer(Player(i), "这是发送给玩家" + I2S(i+1) + "的追踪日志");
				DebugToPlayer(Player(i), "这是发送给玩家" + I2S(i+1) + "的调试日志");
				InfoToPlayer(Player(i), "这是发送给玩家" + I2S(i+1) + "的信息日志");
				WarnToPlayer(Player(i), "这是发送给玩家" + I2S(i+1) + "的警告日志");
				ErrorToPlayer(Player(i), "这是发送给玩家" + I2S(i+1) + "的错误日志");
			}
		}
		// 显示测试结束信息
		Info("玩家专属日志测试完成");
