...
-- 用于调用编译时,先将CompileTestLibraryIncluced设为define 编译完成后,改回undef
...
function WriteFile(file, fileTab)
    ...
    for i, line in ipairs(fileTab) do
        if (string.match(line, "^%s*#undef%s*CompileTestLibraryIncluced") or
            string.match(line, "^%s*#define%s*CompileTestLibraryIncluced")) then
            file:write("#define CompileTestLibraryIncluced")
        else
            file:write(line)
        end
        file:write("\n")
