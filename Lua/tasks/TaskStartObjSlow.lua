...
local function rebuildObjCache()
	...
	if not copyOk then
		print("[Obj慢启]构建缓存失败,复制错误: " .. tostring(copyMsg))
		return false
	end
	os.remove(map)
	print("[Obj慢启]构建缓存完成: " .. cacheMap)
	return true
end
...
local function replaceMapScriptWithStorm()
	...
	if not fu.fileExist(w2lLuaExe) then
		print("[Obj慢启]未找到w3x2lni-lua.exe: " .. w2lLuaExe)
		return false
	end
	if not fu.fileExist(stormTask) then
		print("[Obj慢启]未找到Storm替换脚本: " .. stormTask)
		return false
	end
	if not fu.fileExist(cacheMap) then
		print("[Obj慢启]未找到Obj缓存地图: " .. cacheMap)
		return false
	end
	if not fu.fileExist(path.CompileResult) then
		print("[Obj慢启]未找到编译输出脚本: " .. path.CompileResult)
		return false
	end
