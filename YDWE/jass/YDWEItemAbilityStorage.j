...
function YDWEItemAbilitySystemByAbility_CON takes nothing returns boolean
    ...
    local trigger trg=YDWEGetTriggerByString(I2S(YDWEH2I(u_hero)), "MulItemSlotByAbility")
    ...
    local boolean OnOff=YDWEGetBooleanByString(I2S(YDWEH2I(u_hero)), "YDWEItemAbilitySystemTriggerClose")
    ...
    loop
        ...
        if GetTriggerEventId()==EVENT_PLAYER_UNIT_DROP_ITEM then
            call UnitRemoveAbility(u_hero,skills[i])
          elseif GetTriggerEventId()==EVENT_PLAYER_UNIT_PICKUP_ITEM then
            call UnitAddAbility(u_hero,skills[i])
            call UnitMakeAbilityPermanent(u_hero,true,skills[i])
        endif
    endloop
    set it=null
    set u_hero=null
