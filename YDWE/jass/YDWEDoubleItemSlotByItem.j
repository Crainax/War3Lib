...
function YDWEDoubleItemSlotByAbility_CON takes nothing returns boolean
    ...
    local unit u_hero = GetTriggerUnit()
    ...
    call YDWEItemSwitch(u_hero,u)
    call SetUnitAbilityLevel(u_hero,ab,i)
    if i==1 then
...
function YDWEDoubleItemSlotByAbility takes unit u_hero,integer ab,integer uid,integer MaxItemSlot returns nothing
    local integer i = YDWEGetIntegerByString(I2S(YDWEH2I(u_hero)),"MulItemSlotByAbility")
    local trigger trg = null
    local unit u=null
    if i==0 then
        set trg = CreateTrigger()
        call TriggerRegisterUnitEvent(trg, u_hero, EVENT_UNIT_SPELL_EFFECT)
        call TriggerAddCondition(trg, Condition(function YDWEDoubleItemSlotByAbility_CON))
        call YDWESaveTriggerByString(I2S(YDWEH2I(u_hero)),"MulItemSlotByAbility",trg)
...
function YDWEDoubleItemSlotByItem_CON takes nothing returns boolean
    ...
    set u=YDWEGetUnitByString(I2S(YDWEH2I(trg)),I2S(i))
    call YDWEItemSwitch(u_hero,u)
    call SetItemCharges(UnitItemInSlot(u_hero,5),i)
    if i==1 then
        call YDWESaveUnitByString(I2S(YDWEH2I(trg)),I2S(ItemSlotMax),u)
      else
        call YDWESaveUnitByString(I2S(YDWEH2I(trg)),I2S(i-1),u)
...
function YDWEDoubleItemSlotByItem takes unit u_hero,integer ItemTypeId,integer uid,integer MaxItemSlot returns nothing
    local integer i = YDWEGetIntegerByString(I2S(YDWEH2I(u_hero)),"MulItemSlotByItem")
    local trigger trg = null
    local unit u=null
    local item it=null
    if i==0 then
        set trg = CreateTrigger()
        call TriggerRegisterUnitEvent(trg, u_hero, EVENT_UNIT_USE_ITEM)
        call TriggerAddCondition(trg, Condition(function YDWEDoubleItemSlotByItem_CON))
        call YDWESaveTriggerByString(I2S(YDWEH2I(u_hero)),"MulItemSlotByItem",trg)
