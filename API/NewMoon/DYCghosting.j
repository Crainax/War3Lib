...
library DYCghosting requires DYCbase, XYwebase
...
function SysghostingTimer1 takes nothing returns nothing
    ...
    local real intt=LoadReal(UDGht,hd,StringHash("ghostingint"))
    ...
    if ( ((d<=3000*0.02)and(d>ms0*0.8*0.02)) or RAbsBJ(h0-h1)>10 )and(n2>=intt)and(j!=0) then
        set n2=0
...
function DYCghosting takes unit u, real intt, real t, integer mjt, player pl, real xf, real yf returns nothing
    ...
    call SaveReal(UDGht,hd,StringHash("ghostingint"),intt)
    if (t<=0) then
        set t=0.5
    endif 
