#!/usr/bin/env python3
"""
检查 Xlimon 物编 ID / Order 冲突。

- 4 位对象 ID 按大小写不敏感比较
- 检查项目表与 War3 原生 db
- 检查 Order 是否已被标记为使用（Other/List_Order.ini 中行尾含 //）
"""
...
DEFAULT_PROJECT_TABLES = ("OriginMap/table", "UnitTestMap/table")
DEFAULT_DB_ROOTS = (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check Xlimon object editor ID/Order conflicts.")
    parser.add_argument(
        "--project-root",
        default=".",
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Relative table dir under project root. Repeatable.",
    )
    parser.add_argument(
        "--db-root",
        default="",
        help="War3 db root path. If omitted, auto-detect from known paths.",
    )
    parser.add_argument(
        "--db-file",
        action="append",
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Order key to check in Other/List_Order.ini. Repeatable.",
    )
    parser.add_argument(
        "--order-list",
        default="Other/List_Order.ini",
...
def collect_ids_in_dir(path: Path, source: str) -> list[FoundID]:
    ...
    for ini in sorted(path.glob("*.ini")):
        found.extend(collect_ids_in_file(ini, source=source))
    return found


def pick_db_root(arg_db_root: str) -> Path | None:
    if arg_db_root:
        p = Path(arg_db_root)
        return p if p.exists() and p.is_dir() else None
    for candidate in DEFAULT_DB_ROOTS:
        p = Path(candidate)
...
def main() -> int:
    ...
    project_tables = tuple(args.project_table) if args.project_table else DEFAULT_PROJECT_TABLES
    ...
    db_root = pick_db_root(args.db_root)
    if db_root is None:
        print("[warn] no db root found; skip native db check")
    else:
        for ini in db_files:
    ...
    # 全量扫描：报告已有大小写冲突（默认仅告警，避免被原生 db 历史数据阻塞）
